...
shows "(dag_lemma_conclusion A M \<Lambda>1 \<Gamma> [] [] while_in_if_true_before_passive_prog.block_1 ns2 s' True)"
using assms
apply (rule dag_rel_block_lemma_compact, simp)
unfolding while_in_if_true_before_cfg_to_dag_prog.block_3_def while_in_if_true_before_passive_prog.block_1_def
...
DagVerifies: "(\<And> m2' s2'. ((red_cfg_multi A M \<Lambda>1 \<Gamma> [] while_in_if_true_before_passive_prog.proc_body ((Inl 1),(Normal ns2)) (m2',s2')) \<Longrightarrow> (s2' \<noteq> Failure)))" and 
IH_anon6_LoopHead: "(loop_ih A M \<Lambda>1 \<Gamma> [] while_in_if_true_before_cfg_to_dag_prog.proc_body [1] [] while_in_if_true_before_ast_to_cfg_prog.post ns1 s' 2 m' j)"
...
using assms
apply (rule dag_rel_block_lemma_compact, simp)
unfolding while_in_if_true_before_cfg_to_dag_prog.block_6_def while_in_if_true_before_passive_prog.block_2_def
apply cfg_dag_rel_tac_single+
apply simp
apply simp
done
...
apply (rule while_in_if_true_before_passive_prog.outEdges_2)
...
"(dag_lemma_assms A \<Lambda>1 \<Gamma> [] [] [] ns1 ns2)"
shows "(dag_lemma_conclusion A M \<Lambda>1 \<Gamma> [] [] while_in_if_true_before_passive_prog.block_4 ns2 s' False)"
using assms
apply (rule dag_rel_block_lemma_compact, simp)
...
DagAssms: "(dag_lemma_assms A \<Lambda>1 \<Gamma> [] [] [] ns1 ns2)" and 
DagVerifies: "(\<And> m2' s2'. ((red_cfg_multi A M \<Lambda>1 \<Gamma> [] while_in_if_true_before_passive_prog.proc_body ((Inl 4),(Normal ns2)) (m2',s2')) \<Longrightarrow> (s2' \<noteq> Failure)))"
shows "(Semantics.valid_configuration A \<Lambda>1 \<Gamma> [] while_in_if_true_before_ast_to_cfg_prog.post m' s')"
apply (rule cfg_dag_helper_1[OF Red _ _ DagVerifies DagAssms])
...
apply simp
apply (erule allE[where x=2])
apply ((simp add:while_in_if_true_before_passive_prog.outEdges_4))
apply ((simp add:member_rec(1)))
apply (rule cfg_block_anon4)
apply simp
unfolding dag_lemma_assms_def
...
shows "(mods_contained_in (set [1]) while_in_if_true_before_cfg_to_dag_prog.block_2)"
...
using assms
apply (rule dag_rel_block_lemma_compact, simp)
unfolding while_in_if_true_before_cfg_to_dag_prog.block_2_def while_in_if_true_before_passive_prog.block_5_def
apply cfg_dag_rel_tac_single+
apply simp
...
using assms
apply (rule dag_rel_block_lemma_compact, simp)
unfolding while_in_if_true_before_cfg_to_dag_prog.block_1_def while_in_if_true_before_passive_prog.block_6_def
apply cfg_dag_rel_tac_single+
apply simp
apply simp
done
