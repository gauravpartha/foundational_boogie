theory nested_loop_passive_prog
imports Boogie_Lang.Semantics Boogie_Lang.Util nested_loop_before_ast_to_cfg_prog
begin
definition block_0
  where
    "block_0  = []"
...
definition block_2
  where
    "block_2  = [(Assume (BinOp (Var 4) Gt (Lit (LInt 0)))),(Assume (BinOp (Var 5) Eq (BinOp (Var 4) Sub (Lit (LInt 1))))),(Assert (BinOp (Var 5) Ge (Lit (LInt 0)))),(Assume (Lit (LBool False)))]"
definition block_3
  where
    "block_3  = [(Assume (BinOp (Var 6) Eq (BinOp (Var 2) Sub (Lit (LInt 1))))),(Assert (BinOp (Var 6) Ge (Lit (LInt 0)))),(Assume (Lit (LBool False)))]"
definition block_4
  where
    "block_4  = [(Assume (BinOp (Lit (LInt 0)) Ge (Var 4)))]"
definition block_5
  where
    "block_5  = [(Assume (BinOp (Var 4) Ge (Lit (LInt 0))))]"
definition block_6
  where
    "block_6  = [(Assume (BinOp (Var 2) Gt (Lit (LInt 0)))),(Assert (BinOp (Var 3) Ge (Lit (LInt 0))))]"
definition block_7
  where
    "block_7  = [(Assume (BinOp (Var 2) Ge (Lit (LInt 0))))]"
definition block_8
  where
    "block_8  = [(Assert (BinOp (Lit (LInt 10)) Ge (Lit (LInt 0))))]"
definition block_9
  where
    "block_9  = []"
definition block_10
...
definition outEdges
  where
    "outEdges  = [[],[0],[0],[0],[3],[4,2],[5],[1,6],[7],[8],[9]]"
...
lemma outEdges_7:
shows "((nth (out_edges proc_body) 7) = [1,6])"
by (simp add:proc_body_def outEdges_def)
...
definition locals_vdecls :: "(vdecls)"
  where
    "locals_vdecls  = [(0,(TPrim TInt),(None )),(1,(TPrim TInt),(None )),(2,(TPrim TInt),(None )),(3,(TPrim TInt),(None )),(4,(TPrim TInt),(None )),(5,(TPrim TInt),(None )),(6,(TPrim TInt),(None ))]"
lemma locals_min_aux:
shows "(((map fst (append nested_loop_passive_prog.params_vdecls nested_loop_passive_prog.locals_vdecls)) \<noteq> []) \<longrightarrow> ((Min (set (map fst (append nested_loop_passive_prog.params_vdecls nested_loop_passive_prog.locals_vdecls)))) \<ge> 0))"
unfolding nested_loop_passive_prog.params_vdecls_def nested_loop_passive_prog.locals_vdecls_def
by simp

lemma locals_min:
shows "(\<forall> x. ((Set.member x (set (map fst (append nested_loop_passive_prog.params_vdecls nested_loop_passive_prog.locals_vdecls)))) \<longrightarrow> (x \<ge> 0)))"
using locals_min_aux helper_min
by blast

lemma globals_locals_disj:
shows "((Set.inter (set (map fst (append nested_loop_before_ast_to_cfg_prog.constants_vdecls nested_loop_before_ast_to_cfg_prog.globals_vdecls))) (set (map fst (append nested_loop_passive_prog.params_vdecls nested_loop_passive_prog.locals_vdecls)))) = {})"
unfolding nested_loop_before_ast_to_cfg_prog.constants_vdecls_def nested_loop_before_ast_to_cfg_prog.globals_vdecls_def
by simp

lemma m_x:
shows "((map_of (append nested_loop_passive_prog.params_vdecls nested_loop_passive_prog.locals_vdecls) 0) = (Some ((TPrim TInt),(None ))))"
by (simp add:params_vdecls_def locals_vdecls_def)

lemma m_y:
shows "((map_of (append nested_loop_passive_prog.params_vdecls nested_loop_passive_prog.locals_vdecls) 1) = (Some ((TPrim TInt),(None ))))"
by (simp add:params_vdecls_def locals_vdecls_def)

lemma m_x_0:
shows "((map_of (append nested_loop_passive_prog.params_vdecls nested_loop_passive_prog.locals_vdecls) 2) = (Some ((TPrim TInt),(None ))))"
by (simp add:params_vdecls_def locals_vdecls_def)

lemma m_y_0:
shows "((map_of (append nested_loop_passive_prog.params_vdecls nested_loop_passive_prog.locals_vdecls) 3) = (Some ((TPrim TInt),(None ))))"
by (simp add:params_vdecls_def locals_vdecls_def)

lemma m_y_1:
shows "((map_of (append nested_loop_passive_prog.params_vdecls nested_loop_passive_prog.locals_vdecls) 4) = (Some ((TPrim TInt),(None ))))"
by (simp add:params_vdecls_def locals_vdecls_def)

lemma m_y_2:
shows "((map_of (append nested_loop_passive_prog.params_vdecls nested_loop_passive_prog.locals_vdecls) 5) = (Some ((TPrim TInt),(None ))))"
by (simp add:params_vdecls_def locals_vdecls_def)

lemma m_x_1:
shows "((map_of (append nested_loop_passive_prog.params_vdecls nested_loop_passive_prog.locals_vdecls) 6) = (Some ((TPrim TInt),(None ))))"
by (simp add:params_vdecls_def locals_vdecls_def)

lemma l_x:
shows "((lookup_var_decl ((append nested_loop_before_ast_to_cfg_prog.constants_vdecls nested_loop_before_ast_to_cfg_prog.globals_vdecls),(append nested_loop_passive_prog.params_vdecls nested_loop_passive_prog.locals_vdecls)) 0) = (Some ((TPrim TInt),(None ))))" and "((lookup_var_ty ((append nested_loop_before_ast_to_cfg_prog.constants_vdecls nested_loop_before_ast_to_cfg_prog.globals_vdecls),(append nested_loop_passive_prog.params_vdecls nested_loop_passive_prog.locals_vdecls)) 0) = (Some (TPrim TInt)))"
