...
lemma endblock_skip:
  assumes "A,M,\<Lambda>,\<Gamma>,\<Omega>,T \<turnstile>(BigBlock None [] None None, KEndBlock (KSeq bb_next cont0), Normal ns3) -n\<longrightarrow>^l (reached_bb, reached_cont, reached_state)"
  shows "(valid_configuration A \<Lambda> \<Gamma> \<Omega> [] reached_bb reached_cont reached_state) \<or> 
...
proof (cases l)
  case 0
  then show ?thesis by (metis Ast.valid_configuration_def assms get_state.simps is_final.simps(6) relpowp_fun_conv state.simps(3))
next
  case 1: (Suc l1)
  then show ?thesis 
  proof (cases l1)
...
lemma endblock_skip_wrapper:
  assumes "A,M,\<Lambda>,\<Gamma>,\<Omega>,T \<turnstile>(BigBlock None [] None None, KEndBlock (KSeq (BigBlock name [] (Some (WhileWrapper str)) tr) cont0), Normal ns3) -n\<longrightarrow>^l (reached_bb, reached_cont, reached_state)"
  shows "(valid_configuration A \<Lambda> \<Gamma> \<Omega> [] reached_bb reached_cont reached_state) \<or> 
...
lemma block_local_rel_generic:
  assumes block_rel: "ast_cfg_rel guard invs (BigBlock name cs1 any_str any_tr) cs2"
  and "cs1 \<noteq> Nil"
  and "cs2 \<noteq> Nil"
  and "guard = None"
  and "invs = []"
  and Red_bb_to: "red_bigblock A M \<Lambda> \<Gamma> \<Omega> T ((BigBlock name cs1 any_str any_tr), cont0, (Normal ns1)) (reached_bb, reached_cont, reached_state)" 
...
proof (induction arbitrary: ns1)
  case (Rel_Main_test name cs1 any_str any_tr)
  thus ?case
  proof (cases cs1)
    case Nil
    then show ?thesis using Rel_Main_test(2) by simp
  next
    case (Cons a list)
    ...
    then show ?thesis using Rel_Main_test(6) by auto
  qed 
qed auto
...
lemma block_local_rel_guard_true:
  assumes block_rel: "ast_cfg_rel None [] (BigBlock name cs1 any_str any_tr) cs2"
  and "cs1 \<noteq> Nil"
  and "cs2 \<noteq> Nil"
...
proof cases
  ...
  have Red_impl_extended: "(\<And> s2'.((red_cmd_list A M \<Lambda> \<Gamma> \<Omega> cs2 (Normal ns1) s2') \<Longrightarrow> (s2' \<noteq> Failure)))" 
    using trace_is_possible \<open>c = Assume block_guard\<close> Red_impl RedAssumeOk RedCmdListCons by blast
  hence snd_step_to_end: "reached_state \<noteq> Failure  \<and> (\<forall>ns1'. reached_state = Normal ns1' \<longrightarrow> (A,M,\<Lambda>,\<Gamma>,\<Omega> \<turnstile> \<langle>cs2, Normal ns1\<rangle> [\<rightarrow>] Normal ns1'))"
    using Red_bb_to assms(2-3) block_local_rel_generic block_rel 
    by blast 
...
lemma block_local_rel_loop_head:
  assumes block_rel: "ast_cfg_rel None assert_invs (BigBlock name [] (Some (ParsedWhile loop_guard invs (bb0#body_bbs))) any_tr) assertions"
  and "assert_invs = map inv_into_assertion invs"
  and Red_bb: "red_bigblock A M \<Lambda> \<Gamma> \<Omega> T ((BigBlock name [] (Some (ParsedWhile loop_guard invs (bb0#body_bbs))) any_tr), cont0, (Normal ns1)) (reached_bb, reached_cont, reached_state)" 
...
proof cases
  case Rel_Invs
  hence "assertions = map inv_into_assertion invs" using assms(2) by simp
  from Red_bb show ?thesis
  proof cases
    case RedParsedWhileTrue thus ?thesis using \<open>assertions = (map inv_into_assertion invs)\<close>  by (simp add: asserts_hold_if_invs_hold)
  next
    case RedParsedWhileFalse thus ?thesis using \<open>assertions = (map inv_into_assertion invs)\<close>  by (simp add: asserts_hold_if_invs_hold)
  next
    case RedParsedWhile_InvFail thus ?thesis using Red_impl \<open>assertions = map inv_into_assertion invs\<close> one_inv_fails_assertions by blast
  qed auto
next
  case Rel_Main_test
  hence "assertions = map inv_into_assertion invs" using assms(2) by simp
...
lemma ending:
  assumes "j = Suc j'" 
      and "A,M,\<Lambda>,\<Gamma>,\<Omega>,T \<turnstile>(BigBlock None [] None None, KEndBlock (KSeq bigblock_next KStop), Normal ns1'') -n\<longrightarrow>^j' (reached_bb, reached_cont, reached_state)" 
      ...
      and "\<And> j''. 
            ...
            A,M,\<Lambda>,\<Gamma>,\<Omega>,T \<turnstile>(bigblock_next, KStop, Normal ns1'') -n\<longrightarrow>^j'' (reached_bb, reached_cont, reached_state) \<Longrightarrow>
           ...
           (A,\<Lambda>,\<Gamma>,\<Omega> \<turnstile> \<langle>UnOp Not guard,ns1''\<rangle> \<Down> BoolV True) \<Longrightarrow> (valid_configuration A \<Lambda> \<Gamma> \<Omega> [] reached_bb reached_cont reached_state)"
    shows "valid_configuration A \<Lambda> \<Gamma> \<Omega> [] reached_bb reached_cont reached_state"
proof -
  from assms(2) have disj_a:
    "(valid_configuration A \<Lambda> \<Gamma> \<Omega> [] reached_bb reached_cont reached_state) \<or> 
       (\<exists> l2. (A,M,\<Lambda>,\<Gamma>,\<Omega>,T \<turnstile>(bigblock_next, KStop, Normal ns1'') -n\<longrightarrow>^l2 (reached_bb, reached_cont, reached_state)) \<and> (j' = Suc (Suc l2)) )" 
    by (simp add: endblock_skip)
  thus ?thesis
  proof cases
    assume "(valid_configuration A \<Lambda> \<Gamma> \<Omega> [] reached_bb reached_cont reached_state)" thus ?thesis by simp
  next 
    assume "\<not> ((valid_configuration A \<Lambda> \<Gamma> \<Omega> [] reached_bb reached_cont reached_state))"
    hence "(\<exists> l2. (A,M,\<Lambda>,\<Gamma>,\<Omega>,T \<turnstile>(bigblock_next, KStop, Normal ns1'') -n\<longrightarrow>^l2 (reached_bb, reached_cont, reached_state)) \<and> (j' = Suc (Suc l2)) )" 
      using disj_a by blast
    thus ?thesis
    proof -
      obtain l2_conc where conc_trace: "(A,M,\<Lambda>,\<Gamma>,\<Omega>,T \<turnstile>(bigblock_next, KStop, Normal ns1'') -n\<longrightarrow>^l2_conc (reached_bb, reached_cont, reached_state))" and 
                           succ_rel: "(j' = Suc (Suc l2_conc))"
        using \<open>\<exists>l2. (A,M,\<Lambda>,\<Gamma>,\<Omega>,T \<turnstile>(bigblock_next, KStop, Normal ns1'') -n\<longrightarrow>^l2 (reached_bb, reached_cont, reached_state)) \<and> j' = Suc (Suc l2)\<close> by blast
      show ?thesis
        apply (rule assms(5))
           apply (rule succ_rel)
         ...
         apply (rule assms(3))
         apply (simp)
        using assms(4) false_equals_not_true 
        by blast
    qed
  qed
qed
...
lemma ending2:
  assumes "j = Suc j'"
      and "A,M,\<Lambda>,\<Gamma>,\<Omega>,T \<turnstile>(BigBlock None [] None None,
      ...
      and "\<And> j'''.
          ...
          (Ast.valid_configuration A \<Lambda> \<Gamma> \<Omega> [] reached_bb reached_cont reached_state)"
...
lemma generic_ending_block_after_loop_global_rel:
  assumes syn_rel: "ast_cfg_rel None [] (BigBlock name cs1 None any_tr) cs2"
  and "(any_tr = None) \<or> (any_tr = (Some (Return val1)))"
  and "cs1 \<noteq> Nil"
  and "cs2 \<noteq> Nil"
  and "node_to_block G ! n = (c#cs2)" 
  and "c = Assume b"
  ...
  and j_step_ast_trace: "A,M,\<Lambda>,\<Gamma>,\<Omega>,T \<turnstile> ((BigBlock name cs1 None any_tr), KStop, (Normal ns1)) -n\<longrightarrow>^j (reached_bb, reached_cont, reached_state)"
  and cfg_is_correct: "\<And> m' s'. (red_cfg_multi A M \<Lambda> \<Gamma> \<Omega> G ((Inl n),(Normal ns1)) (m',s')) \<Longrightarrow> (s' \<noteq> Failure)"
  shows "(valid_configuration A \<Lambda> \<Gamma> \<Omega> [] reached_bb reached_cont reached_state)" 
  using assms
proof (cases j)
  case 0
  from this j_step_ast_trace have "(reached_bb, reached_cont, reached_state) = ((BigBlock name cs1 None any_tr), KStop, (Normal ns1))" by simp
  then show ?thesis by (simp add: valid_configuration_def expr_all_sat_def)
next
  case (Suc j')
  from this j_step_ast_trace obtain inter_bb inter_cont inter_state where
  ...
  proof (cases any_tr)
    ...
    proof cases
      case RedSimpleCmds thus ?thesis using Red_impl trace_is_possible
        by (metis assms(4) assms(7) assms(6) block_local_rel_guard_false step syn_rel)
    qed auto
  ...
  next
    ...
    proof (cases transfer)
      case (Goto x1)
      then show ?thesis using Some assms(2) by blast
    next
      case (Return ret)
...
next
  ...
  proof (cases any_tr)
    ...
    have Red_impl: "(\<And> s2'.((red_cmd_list A M \<Lambda> \<Gamma> \<Omega> cs2 (Normal ns1) s2') \<Longrightarrow> (s2' \<noteq> Failure)))"
      using assms(6) cfg_is_correct dag_verifies_propagate_2 by blast

    from step have "inter_state \<noteq> Failure"
    proof cases
      case RedSimpleCmds thus ?thesis using Red_impl assms(5) block_local_rel_generic local.step syn_rel by blast
    qed auto
  ...
  next
    ...
    next
      ...
      proof cases
        case RedSimpleCmds thus ?thesis using Return Some by blast
      qed (auto simp add: assms(4))
  
      have Red_impl: "(\<And> s2'.((red_cmd_list A M \<Lambda> \<Gamma> \<Omega> cs2 (Normal ns1) s2') \<Longrightarrow> (s2' \<noteq> Failure)))" 
        using dag_verifies_propagate_2  assms(6) cfg_is_correct by blast 
  
      from step have "inter_state \<noteq> Failure"
      proof cases
        case RedSimpleCmds thus ?thesis using Red_impl using assms(5) block_local_rel_generic local.step syn_rel by blast
      qed auto
...
lemma block_global_rel_while_successor:
  assumes j_step_ast_trace: 
      "A,M,\<Lambda>,\<Gamma>,\<Omega>,T \<turnstile> ((BigBlock name cmds (Some (WhileWrapper (ParsedWhile guard invs (body_bb0#body_bbs)))) None), cont1, Normal ns1) -n\<longrightarrow>^j 
                      (reached_bb, reached_cont, reached_state)"
  and syn_rel: "ast_cfg_rel None [] (BigBlock name cmds (Some (WhileWrapper (ParsedWhile guard invs (body_bb0#body_bbs)))) None) cmds"
  and "cmds \<noteq> []"
  and "node_to_block G ! n = cmds"
  and cfg_is_correct: "\<And> m' s'. (red_cfg_multi A M \<Lambda> \<Gamma> \<Omega> G ((Inl n),(Normal ns1)) (m',s')) \<Longrightarrow> (s' \<noteq> Failure)"
  and block_local_rel:
    "\<And> reached_bb_inter reached_cont_inter reached_state_inter. 
    (red_bigblock A M \<Lambda> \<Gamma> \<Omega> T ((BigBlock name cmds (Some (WhileWrapper (ParsedWhile guard invs (body_bb0#body_bbs)))) None), cont1, (Normal ns1)) (reached_bb_inter, reached_cont_inter, reached_state_inter)) \<Longrightarrow> 
    (\<And> s2'.((red_cmd_list A M \<Lambda> \<Gamma> \<Omega> cmds (Normal ns1) s2') \<Longrightarrow> (s2' \<noteq> Failure))) \<Longrightarrow>   
    (reached_state_inter \<noteq> Failure  \<and> (\<forall>ns1'. reached_state_inter = Normal ns1' \<longrightarrow> (A,M,\<Lambda>,\<Gamma>,\<Omega> \<turnstile> \<langle>cmds, Normal ns1\<rangle> [\<rightarrow>] Normal ns1')))"
  and global_rel_succ:
        "\<And> ns2 k.
         k < j \<Longrightarrow>
         ...
         A,M,\<Lambda>,\<Gamma>,\<Omega>,T \<turnstile> ((BigBlock name [] (Some (ParsedWhile guard invs (body_bb0#body_bbs))) None), KEndBlock cont1, Normal ns2) -n\<longrightarrow>^k 
                      (reached_bb, reached_cont, reached_state) \<Longrightarrow> 
         (valid_configuration A \<Lambda> \<Gamma> \<Omega> [] reached_bb reached_cont reached_state)"
  shows "(valid_configuration A \<Lambda> \<Gamma> \<Omega> [] reached_bb reached_cont reached_state)"
  using assms
proof cases
  assume "j = 0"
  then have "(reached_bb, reached_cont, reached_state) = 
             ((BigBlock name cmds (Some (WhileWrapper (ParsedWhile guard invs (body_bb0#body_bbs)))) None), cont1, Normal ns1)" using j_step_ast_trace by auto
...
next
  ...
  have Red_impl: "(\<And> s2'.((red_cmd_list A M \<Lambda> \<Gamma> \<Omega> cmds (Normal ns1) s2') \<Longrightarrow> (s2' \<noteq> Failure)))" using dag_verifies_propagate_2 cfg_is_correct assms(4) by blast
  have local_conclusion: "inter_state \<noteq> Failure  \<and> (\<forall>ns1'. inter_state = Normal ns1' \<longrightarrow> (A,M,\<Lambda>,\<Gamma>,\<Omega> \<turnstile> \<langle>cmds, Normal ns1\<rangle> [\<rightarrow>] Normal ns1'))" 
    using Red_impl first_step assms(3) block_local_rel_generic syn_rel by blast 
  show ?thesis 
  proof (cases inter_state)
    case (Normal x1)
    ...
    proof (cases j')
      case 0
      then show ?thesis 
        by (metis (full_types, opaque_lifting) Ast.valid_configuration_def expr_all_sat_def get_state.simps list.pred_inject(1) local_conclusion relpowp.simps(1) rest)
    next
      case 2: (Suc j'')
      ...
      hence Red_cfg_conc: 
        ...
        using dag_verifies_propagate Normal \<open>A,M,\<Lambda>,\<Gamma>,\<Omega> \<turnstile> \<langle>cmds,Normal ns1\<rangle> [\<rightarrow>] inter_state\<close> assms(4) cfg_is_correct 
        by blast

      from 2 j_step_ast_trace obtain inter_bb2 inter_cont2 inter_state2 where
      ...
      then show ?thesis using a3 rest_2 Normal Red_cfg_conc assms(7) by blast
    qed
  next
    case Failure
...
lemma block_global_rel_loop_head:
  assumes block_rel: "ast_cfg_rel None assertions (BigBlock name [] any_str any_tr) assertions"
  and ast_trace: "A,M,\<Lambda>,\<Gamma>,\<Omega>,T \<turnstile> ((BigBlock name [] any_str any_tr), KEndBlock cont1, (Normal ns1)) -n\<longrightarrow>^j (reached_bb, reached_cont, reached_state)"
  ...
  and block_local_rel:
    "\<And> reached_bb_inter reached_cont_inter reached_state_inter. (red_bigblock A M \<Lambda> \<Gamma> \<Omega> T ((BigBlock name [] any_str any_tr), KEndBlock cont1, (Normal ns1)) (reached_bb_inter, reached_cont_inter, reached_state_inter)) \<Longrightarrow> 
  ...
  (* TODO: The following is a sketchy assumption! Fix it!*)
  and "assertions \<noteq> []" 
  ...
  and succ_correct: 
   "\<And> ns1'' loop_guard j'. 
    ...
    ((cont_guard = Some loop_guard) \<and> 
      ...
      A,M,\<Lambda>,\<Gamma>,\<Omega>,T \<turnstile> (bb0, convert_list_to_cont (rev ((BigBlock name [] any_str any_tr)#body_bbs)) (KEndBlock cont1), (Normal ns1'')) -n\<longrightarrow>^j' (reached_bb, reached_cont, reached_state)) \<or> 
    ((cont_guard = Some loop_guard) \<and> 
    ...
    ((cont_guard = None) \<and> 
     ((A,M,\<Lambda>,\<Gamma>,\<Omega>,T \<turnstile> ((BigBlock name [] None None), KEndBlock cont1, (Normal ns1'')) -n\<longrightarrow>^j' (reached_bb, reached_cont, reached_state)) \<or>
      (A,M,\<Lambda>,\<Gamma>,\<Omega>,T \<turnstile> (bb0, convert_list_to_cont (rev ((BigBlock name [] any_str any_tr)#body_bbs)) (KEndBlock cont1), (Normal ns1'')) -n\<longrightarrow>^j' (reached_bb, reached_cont, reached_state))))  \<Longrightarrow>  
...
proof cases
    ...
    proof cases
      assume "j = 0"
      hence "(reached_bb, reached_cont, reached_state) = ((BigBlock name [] any_str any_tr), KEndBlock cont1, (Normal ns1))" using ast_trace by simp 
      thus ?thesis by (simp add: Ast.valid_configuration_def)
    next
      assume "j \<noteq> 0" 
      from this obtain j' where "j = Suc j'" using not0_implies_Suc by blast
  
      from ast_trace this obtain inter_bb inter_cont inter_state where
      ...
      proof (cases cont_guard)
        ...
        proof cases
          ...
          hence concrete_inter1: "(inter_bb, inter_cont, inter_state) = (bb0, convert_list_to_cont (rev ((BigBlock name [] any_str any_tr)#body_bbs)) (KEndBlock cont1), (Normal ns1))"
          ...
          have succ_cfg_correct: "(\<forall>msuc2.  List.member (out_edges(G) ! n) msuc2 \<longrightarrow> (\<forall>m3 s3. ((A,M,\<Lambda>,\<Gamma>,\<Omega>,G \<turnstile> (Inl(msuc2), inter_state) -n\<rightarrow>* (m3, s3)) \<longrightarrow> s3 \<noteq> Failure)))"
            by (metis Pair_inject assms(7) block_local_rel cfg_correct concrete_inter1 dag_verifies_propagate dag_verifies_propagate_2)
  
          show ?thesis using \<open>j = Suc j'\<close> succ_cfg_correct None rest concrete_inter1 succ_correct by blast 
        next 
          ...
          from first_step
          have succ_cfg_correct: "(\<forall>msuc2.  List.member (out_edges(G) ! n) msuc2 \<longrightarrow> (\<forall>m3 s3. ((A,M,\<Lambda>,\<Gamma>,\<Omega>,G \<turnstile> (Inl(msuc2), inter_state) -n\<rightarrow>* (m3, s3)) \<longrightarrow> s3 \<noteq> Failure)))"
            by (metis assms(7) block_local_rel cfg_correct dag_verifies_propagate dag_verifies_propagate_2 local.RedParsedWhileFalse(5))
  
          show ?thesis using \<open>j = Suc j'\<close> succ_cfg_correct None rest concrete_inter2 succ_correct by blast
        next 
          case RedParsedWhile_InvFail thus ?thesis using assms(7) block_local_rel cfg_correct dag_verifies_propagate_2 first_step by blast
        qed auto
      next
        ...
        proof cases
          ...
          proof cases
            case RedParsedWhileTrue
            hence concrete_inter3: "(inter_bb, inter_cont, inter_state) = (bb0, convert_list_to_cont (rev ((BigBlock name [] any_str any_tr)#body_bbs)) (KEndBlock cont1), (Normal ns1))"
              using bb_successor_while Some by blast
  
            from first_step
            have succ_cfg_correct: "(\<forall>msuc2.  List.member (out_edges(G) ! n) msuc2 \<longrightarrow> (\<forall>m3 s3. ((A,M,\<Lambda>,\<Gamma>,\<Omega>,G \<turnstile> (Inl(msuc2), inter_state) -n\<rightarrow>* (m3, s3)) \<longrightarrow> s3 \<noteq> Failure)))"
              by (metis Pair_inject assms(7) block_local_rel cfg_correct concrete_inter3 dag_verifies_propagate dag_verifies_propagate_2)
  
            show ?thesis using \<open>j = Suc j'\<close> succ_cfg_correct Some guard_true rest concrete_inter3 succ_correct by blast 
        next 
            case RedParsedWhile_InvFail thus ?thesis using assms(7) block_local_rel cfg_correct dag_verifies_propagate_2 first_step by blast
...
lemma block_global_rel_if_successor:
  assumes block_rel: "ast_cfg_rel None [] (BigBlock name cs1 any_str any_tr) cs2"
  and ast_trace: "A,M,\<Lambda>,\<Gamma>,\<Omega>,T \<turnstile> ((BigBlock name cs1 any_str any_tr), cont0, (Normal ns1)) -n\<longrightarrow>^j (reached_bb, reached_cont, reached_state)"
  and "cs1 \<noteq> Nil"
  and "cs2 \<noteq> Nil"
  and "node_to_block(G) ! n = cs2"
  and cfg_correct: "(\<And> m2 s2. ((red_cfg_multi A M \<Lambda> \<Gamma> \<Omega> G (Inl n, Normal ns1) (m2, s2)) \<Longrightarrow> (s2 \<noteq> Failure)))"
...
proof cases
  ...
  proof (cases j)
    case 0
    hence "(reached_bb, reached_cont, reached_state) = ((BigBlock name cs1 any_str any_tr), cont0, (Normal ns1))" using ast_trace by auto
    then show ?thesis by (simp add: Ast.valid_configuration_def bb_successor_if) 
  next
    case 1: (Suc j')
    from this obtain inter_bb inter_cont inter_state where
      ...
      next
        ...
        proof (cases cont_guard)
          ...
          proof cases
            ...
            have succ_cfg_correct: "(\<forall>msuc2.  List.member (out_edges(G) ! n) msuc2 \<longrightarrow> (\<forall>m3 s3. ((A,M,\<Lambda>,\<Gamma>,\<Omega>,G \<turnstile> (Inl(msuc2), inter_state) -n\<rightarrow>* (m3, s3)) \<longrightarrow> s3 \<noteq> Failure)))"
            proof cases
              case RedSimpleCmds show ?thesis using 2 eq RedSimpleCmds(3) dag_verifies_propagate assms(5) Rel_Main_test(1) cfg_correct by blast
...
proof cases
  ...
  proof (cases j)
    case 0
    hence "(reached_bb, reached_cont, reached_state) = ((BigBlock name cs1 any_str any_tr), cont0, (Normal ns1))" using ast_trace by fastforce
      ...
      next
        ...
        proof cases
          ...
          hence cmds_red: "A,M,\<Lambda>,\<Gamma>,\<Omega> \<turnstile> \<langle>c#cs2, Normal ns1\<rangle> [\<rightarrow>] inter_state" 
            using Rel_Main_test(1) \<open>c = Assume some_cmd\<close> \<open>(UnOp Not block_guard) \<sim> some_cmd\<close> equiv_preserves_value trace_is_possible RedAssumeOk RedCmdListCons 
            by metis
          show ?thesis by (metis (no_types, lifting) "1" RedNormalSucc assms(5) assms(6) cfg_correct cmds_red converse_rtranclp_into_rtranclp)
        qed (auto simp add: \<open>cs1 \<noteq> Nil\<close>) 
...
proof cases
  ...
  proof (cases j)
    case 0
    hence "(reached_bb, reached_cont, reached_state) = ((BigBlock name cs1 any_str any_tr), cont0, (Normal ns1))" using ast_trace by auto
    then show ?thesis unfolding Ast.valid_configuration_def by (metis assms(3) get_state.simps is_final.simps(2) neq_Nil_conv state.distinct(1))
  next
    case succ_0: (Suc j')
    from this obtain inter_bb inter_cont inter_state where
      first_step: "red_bigblock A M \<Lambda> \<Gamma> \<Omega> T ((BigBlock name cs1 any_str any_tr), cont0, (Normal ns1)) (inter_bb, inter_cont, inter_state)" and
    ...
    proof (cases inter_state)
      ...
      next
        ...
        proof cases
          case RedSimpleCmds 
          hence cmds_red: "A,M,\<Lambda>,\<Gamma>,\<Omega> \<turnstile> \<langle>c#cs2, Normal ns1\<rangle> [\<rightarrow>] inter_state" 
            using Rel_Main_test(1) \<open>c = Assume block_guard\<close> trace_is_possible RedAssumeOk RedCmdListCons by blast
          show ?thesis using "1" assms(5) assms(6) cfg_correct cmds_red dag_verifies_propagate by blast
        qed (auto simp add: \<open>cs1 \<noteq> Nil\<close>) 
  ...
  next
    ...
    proof (cases inter_state)
      ...
      next
        ...
        have succ_cfg_correct: "(\<forall>msuc2.  List.member (out_edges(G) ! n) msuc2 \<longrightarrow> (\<forall>m3 s3. ((A,M,\<Lambda>,\<Gamma>,\<Omega>,G \<turnstile> (Inl(msuc2), inter_state) -n\<rightarrow>* (m3, s3)) \<longrightarrow> s3 \<noteq> Failure)))"
        proof cases
          case RedSimpleCmds show ?thesis using 1 snd_step_equiv  RedSimpleCmds(3) dag_verifies_propagate Rel_Main_test(1) cfg_correct assms(5) by blast
        qed (auto simp add: \<open>cs1 \<noteq> Nil\<close>) 
