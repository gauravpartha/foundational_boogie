...
shows "(expr_all_sat A \<Lambda>1 \<Gamma> [] ns2 p_before_ast_to_cfg_prog.post)"
unfolding expr_all_sat_def p_before_ast_to_cfg_prog.post_def 
...
shows "(mods_contained_in (set []) p_before_cfg_to_dag_prog.block_2)"
...
apply simp
apply simp
apply (erule type_safety_top_level_inv[OF Wf_Fun p_before_ast_to_cfg_prog.funcs_wf p_before_ast_to_cfg_prog.var_context_wf])
apply (simp)
apply ((tactic \<open> typing_tac @{context} [] @{thms p_before_ast_to_cfg_prog.l_x(2)} [] 1\<close>))
...
IH_anon3_LoopHead: "(loop_ih A M \<Lambda>1 \<Gamma> [] p_before_cfg_to_dag_prog.proc_body [] [(BinOp (Var 0) Ge (Lit (LInt 0)))] p_before_ast_to_cfg_prog.post ns1 s' 1 m' j)"
...
using assms
apply (rule dag_rel_block_lemma_compact, simp)
unfolding p_before_cfg_to_dag_prog.block_4_def p_before_passive_prog.block_2_def
apply cfg_dag_rel_tac_single+
apply simp
apply simp
done
...
using assms
apply (rule dag_rel_block_lemma_compact, simp)
unfolding p_before_cfg_to_dag_prog.block_3_def p_before_passive_prog.block_3_def
apply cfg_dag_rel_tac_single+
apply simp
apply simp
done
...
shows "(mods_contained_in (set []) p_before_cfg_to_dag_prog.block_1)"
...
"(dag_lemma_assms A \<Lambda>1 \<Gamma> [] [] [(BinOp (Var 0) Ge (Lit (LInt 0)))] ns1 ns2)"
shows "(dag_lemma_conclusion A M \<Lambda>1 \<Gamma> [] [] p_before_passive_prog.block_4 ns2 s' False)"
using assms
apply (rule dag_rel_block_lemma_compact, simp)
unfolding p_before_cfg_to_dag_prog.block_1_def p_before_passive_prog.block_4_def
apply cfg_dag_rel_tac_single+
apply simp
apply simp
done
...
DagAssms: "(dag_lemma_assms A \<Lambda>1 \<Gamma> [] [] [(BinOp (Var 0) Ge (Lit (LInt 0)))] ns1 ns2)" and 
...
"(dag_lemma_assms A \<Lambda>1 \<Gamma> [] [] [] ns1 ns2)"
shows "(dag_lemma_conclusion A M \<Lambda>1 \<Gamma> [] [(BinOp (Var 0) Ge (Lit (LInt 0)))] p_before_passive_prog.block_5 ns2 s' False)"
using assms
apply (rule dag_rel_block_lemma_compact, simp)
unfolding p_before_cfg_to_dag_prog.block_0_def p_before_passive_prog.block_5_def
apply cfg_dag_rel_tac_single+
apply simp
apply simp
apply (erule type_safety_top_level_inv[OF Wf_Fun p_before_ast_to_cfg_prog.funcs_wf p_before_ast_to_cfg_prog.var_context_wf])
...
lemma cfg_block_0:
assumes 
"(\<forall> m2' s2'. ((red_cfg_multi A M \<Lambda>1 \<Gamma> [] p_before_passive_prog.proc_body ((Inl 6),(Normal ns2)) (m2',s2')) \<longrightarrow> (s2' \<noteq> Failure)))" and 
"(nstate_same_on \<Lambda>1 ns1 ns2 {})" and 
"(state_well_typed A \<Lambda>1 [] ns1)" and 
"(state_well_typed A \<Lambda>1 [] ns2)" and 
"((\<And> m2' s2'. ((red_cfg_multi A M \<Lambda>1 \<Gamma> [] p_before_passive_prog.proc_body ((Inl 5),(Normal ns2)) (m2',s2')) \<Longrightarrow> (s2' \<noteq> Failure))) \<Longrightarrow> R)"
shows "R"
using assms
apply (rule cfg_dag_empty_propagate_helper)
apply (assumption, simp)
apply ((simp add:p_before_passive_prog.outEdges_6))
by ((simp add:p_before_passive_prog.node_6 p_before_passive_prog.block_6_def))
...
"(\<And> m2' s2'. ((red_cfg_multi A M \<Lambda>1 \<Gamma> [] p_before_passive_prog.proc_body ((Inl 7),(Normal ns2)) (m2',s2')) \<Longrightarrow> (s2' \<noteq> Failure)))" and 
...
apply (erule assms(3))
apply (rule assms(2))
unfolding p_before_passive_prog.block_7_def
apply (rule assume_pres_normal[where ?es=p_before_ast_to_cfg_prog.pres])
apply (rule assms(4))
unfolding p_before_ast_to_cfg_prog.pres_def
apply simp
apply (rule p_before_passive_prog.outEdges_7)
apply ((simp add:p_before_passive_prog.node_6 p_before_passive_prog.block_6_def))
apply (rule p_before_passive_prog.outEdges_6)
