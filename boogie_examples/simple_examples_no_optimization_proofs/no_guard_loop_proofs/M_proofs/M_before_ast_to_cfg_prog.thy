theory M_before_ast_to_cfg_prog
imports Boogie_Lang.Ast Boogie_Lang.Semantics Boogie_Lang.TypeSafety Boogie_Lang.Util "../global_data"
begin
definition bigblock_0
  where
    "bigblock_0  = (BigBlock (None ) [(Havoc 0),(Havoc 1),(Havoc 2)] (Some (WhileWrapper (ParsedWhile (None ) [] [(BigBlock (None ) [] (Some (WhileWrapper (ParsedWhile (Some (BinOp (Var 1) Gt (Lit (LInt 10)))) [] [(BigBlock (None ) [(Assign 0 (BinOp (Var 0) Mul (Lit (LInt 10))))] (None ) (None ))]))) (None )),(BigBlock (None ) [] (Some (ParsedIf (Some (BinOp (BinOp (Var 0) Sub (Lit (LInt 10))) Gt (Lit (LInt 200)))) [(BigBlock (None ) [(Assign 1 (BinOp (Var 2) Add (Lit (LInt 7))))] (None ) (None ))] [(BigBlock (None ) [] (None ) (None ))])) (None )),(BigBlock (None ) [(Assign 0 (BinOp (BinOp (Var 0) Add (Var 1)) Add (Var 2)))] (None ) (None ))]))) (None ))"
definition bigblock_1
  where
    "bigblock_1  = (BigBlock (None ) [] (Some (ParsedWhile (None ) [] [(BigBlock (None ) [] (Some (WhileWrapper (ParsedWhile (Some (BinOp (Var 1) Gt (Lit (LInt 10)))) [] [(BigBlock (None ) [(Assign 0 (BinOp (Var 0) Mul (Lit (LInt 10))))] (None ) (None ))]))) (None )),(BigBlock (None ) [] (Some (ParsedIf (Some (BinOp (BinOp (Var 0) Sub (Lit (LInt 10))) Gt (Lit (LInt 200)))) [(BigBlock (None ) [(Assign 1 (BinOp (Var 2) Add (Lit (LInt 7))))] (None ) (None ))] [(BigBlock (None ) [] (None ) (None ))])) (None )),(BigBlock (None ) [(Assign 0 (BinOp (BinOp (Var 0) Add (Var 1)) Add (Var 2)))] (None ) (None ))])) (None ))"
definition bigblock_2
  where
    "bigblock_2  = (BigBlock (None ) [] (Some (WhileWrapper (ParsedWhile (Some (BinOp (Var 1) Gt (Lit (LInt 10)))) [] [(BigBlock (None ) [(Assign 0 (BinOp (Var 0) Mul (Lit (LInt 10))))] (None ) (None ))]))) (None ))"
definition bigblock_3
  where
    "bigblock_3  = (BigBlock (None ) [] (Some (ParsedWhile (Some (BinOp (Var 1) Gt (Lit (LInt 10)))) [] [(BigBlock (None ) [(Assign 0 (BinOp (Var 0) Mul (Lit (LInt 10))))] (None ) (None ))])) (None ))"
definition bigblock_4
  where
    "bigblock_4  = (BigBlock (None ) [(Assign 0 (BinOp (Var 0) Mul (Lit (LInt 10))))] (None ) (None ))"
definition bigblock_5
  where
    "bigblock_5  = (BigBlock (None ) [] (Some (ParsedIf (Some (BinOp (BinOp (Var 0) Sub (Lit (LInt 10))) Gt (Lit (LInt 200)))) [(BigBlock (None ) [(Assign 1 (BinOp (Var 2) Add (Lit (LInt 7))))] (None ) (None ))] [(BigBlock (None ) [] (None ) (None ))])) (None ))"
definition bigblock_6
  where
    "bigblock_6  = (BigBlock (None ) [(Assign 1 (BinOp (Var 2) Add (Lit (LInt 7))))] (None ) (None ))"
definition bigblock_7
  where
    "bigblock_7  = (BigBlock (None ) [] (None ) (None ))"
definition bigblock_8
  where
    "bigblock_8  = (BigBlock (None ) [(Assign 0 (BinOp (BinOp (Var 0) Add (Var 1)) Add (Var 2)))] (None ) (None ))"
definition bigblock_9
  where
    "bigblock_9  = (BigBlock (None ) [(Assign 2 (BinOp (Var 2) Sub (Lit (LInt 2))))] (None ) (None ))"
definition cont_9
  where
    "cont_9  = KStop"
definition cont_0
...
definition cont_6
  where
    "cont_6  = (KSeq bigblock_8 cont_8)"
definition cont_7
  where
    "cont_7  = (KSeq bigblock_8 cont_8)"
definition cont_2
  where
...
shows "(((map fst (append M_before_ast_to_cfg_prog.constants_vdecls M_before_ast_to_cfg_prog.globals_vdecls)) \<noteq> []) \<longrightarrow> ((Max (set (map fst (append M_before_ast_to_cfg_prog.constants_vdecls M_before_ast_to_cfg_prog.globals_vdecls)))) \<le> 0))"
unfolding M_before_ast_to_cfg_prog.constants_vdecls_def M_before_ast_to_cfg_prog.globals_vdecls_def
by simp

lemma globals_max:
shows "(\<forall> x. ((Set.member x (set (map fst (append M_before_ast_to_cfg_prog.constants_vdecls M_before_ast_to_cfg_prog.globals_vdecls)))) \<longrightarrow> (x \<le> 0)))"
...
lemma consts_wf:
shows "((list_all (comp (wf_ty 0) (comp fst snd)) M_before_ast_to_cfg_prog.constants_vdecls) )"
unfolding M_before_ast_to_cfg_prog.constants_vdecls_def
by simp

lemma globals_wf:
shows "((list_all (comp (wf_ty 0) (comp fst snd)) M_before_ast_to_cfg_prog.globals_vdecls) )"
unfolding M_before_ast_to_cfg_prog.globals_vdecls_def
by simp

lemma params_wf:
shows "((list_all (comp (wf_ty 0) (comp fst snd)) M_before_ast_to_cfg_prog.params_vdecls) )"
unfolding M_before_ast_to_cfg_prog.params_vdecls_def
by simp

lemma locals_wf:
shows "((list_all (comp (wf_ty 0) (comp fst snd)) M_before_ast_to_cfg_prog.locals_vdecls) )"
unfolding M_before_ast_to_cfg_prog.locals_vdecls_def
by simp

lemma var_context_wf:
shows "(\<forall> x \<tau>. (((lookup_var_ty ((append M_before_ast_to_cfg_prog.constants_vdecls M_before_ast_to_cfg_prog.globals_vdecls),(append M_before_ast_to_cfg_prog.params_vdecls M_before_ast_to_cfg_prog.locals_vdecls)) x) = (Some \<tau>)) \<longrightarrow> ((wf_ty 0) \<tau>)))"
apply (rule lookup_ty_pred_2)
by ((simp_all add:consts_wf globals_wf params_wf locals_wf))

lemma m_x:
shows "((map_of (append M_before_ast_to_cfg_prog.params_vdecls M_before_ast_to_cfg_prog.locals_vdecls) 0) = (Some ((TPrim TInt),(None ))))"
by (simp add:params_vdecls_def locals_vdecls_def)

lemma m_y:
shows "((map_of (append M_before_ast_to_cfg_prog.params_vdecls M_before_ast_to_cfg_prog.locals_vdecls) 1) = (Some ((TPrim TInt),(None ))))"
by (simp add:params_vdecls_def locals_vdecls_def)

lemma m_z:
shows "((map_of (append M_before_ast_to_cfg_prog.params_vdecls M_before_ast_to_cfg_prog.locals_vdecls) 2) = (Some ((TPrim TInt),(None ))))"
by (simp add:params_vdecls_def locals_vdecls_def)

lemma l_x:
shows "((lookup_var_decl ((append M_before_ast_to_cfg_prog.constants_vdecls M_before_ast_to_cfg_prog.globals_vdecls),(append M_before_ast_to_cfg_prog.params_vdecls M_before_ast_to_cfg_prog.locals_vdecls)) 0) = (Some ((TPrim TInt),(None ))))" and "((lookup_var_ty ((append M_before_ast_to_cfg_prog.constants_vdecls M_before_ast_to_cfg_prog.globals_vdecls),(append M_before_ast_to_cfg_prog.params_vdecls M_before_ast_to_cfg_prog.locals_vdecls)) 0) = (Some (TPrim TInt)))"
using globals_locals_disj m_x
by (simp_all add: lookup_var_decl_global_2 lookup_var_decl_local lookup_var_decl_ty_Some)

lemma l_y:
shows "((lookup_var_decl ((append M_before_ast_to_cfg_prog.constants_vdecls M_before_ast_to_cfg_prog.globals_vdecls),(append M_before_ast_to_cfg_prog.params_vdecls M_before_ast_to_cfg_prog.locals_vdecls)) 1) = (Some ((TPrim TInt),(None ))))" and "((lookup_var_ty ((append M_before_ast_to_cfg_prog.constants_vdecls M_before_ast_to_cfg_prog.globals_vdecls),(append M_before_ast_to_cfg_prog.params_vdecls M_before_ast_to_cfg_prog.locals_vdecls)) 1) = (Some (TPrim TInt)))"
using globals_locals_disj m_y
by (simp_all add: lookup_var_decl_global_2 lookup_var_decl_local lookup_var_decl_ty_Some)

lemma l_z:
shows "((lookup_var_decl ((append M_before_ast_to_cfg_prog.constants_vdecls M_before_ast_to_cfg_prog.globals_vdecls),(append M_before_ast_to_cfg_prog.params_vdecls M_before_ast_to_cfg_prog.locals_vdecls)) 2) = (Some ((TPrim TInt),(None ))))" and "((lookup_var_ty ((append M_before_ast_to_cfg_prog.constants_vdecls M_before_ast_to_cfg_prog.globals_vdecls),(append M_before_ast_to_cfg_prog.params_vdecls M_before_ast_to_cfg_prog.locals_vdecls)) 2) = (Some (TPrim TInt)))"
using globals_locals_disj m_z
by (simp_all add: lookup_var_decl_global_2 lookup_var_decl_local lookup_var_decl_ty_Some)

definition ast_proc :: "(ast procedure)"
  where
    "ast_proc  = (|proc_ty_args = 0,proc_args = M_before_ast_to_cfg_prog.params_vdecls,proc_rets = [],proc_modifs = [],proc_pres = (exprs_to_only_checked_spec M_before_ast_to_cfg_prog.pres),proc_posts = (exprs_to_only_checked_spec M_before_ast_to_cfg_prog.post),proc_body = (Some (M_before_ast_to_cfg_prog.locals_vdecls,M_before_ast_to_cfg_prog.proc_body))|)"
