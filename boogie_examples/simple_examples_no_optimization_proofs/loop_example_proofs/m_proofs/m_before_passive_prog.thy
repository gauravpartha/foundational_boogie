...
definition block_1
  where
    "block_1  = [(Assume (BinOp (Var 0) Lt (Var 1))),(Assign 0 (BinOp (Var 0) Add (Lit (LInt 1)))),(Assert (BinOp (Var 0) Le (Var 1))),(Assume (Lit (LBool False)))]"
definition block_2
  where
    "block_2  = [(Assert (BinOp (Var 0) Ge (Var 1)))]"
definition block_3
  where
    "block_3  = [(Assume (BinOp (Var 1) Le (Var 0)))]"
definition block_4
  where
    "block_4  = [(Havoc 0),(Assume (BinOp (Var 0) Le (Var 1)))]"
definition block_5
  where
    "block_5  = [(Assign 0 (Lit (LInt 0))),(Assume (BinOp (Var 1) Gt (Lit (LInt 0)))),(Assert (BinOp (Var 0) Le (Var 1)))]"
definition block_6
  where
    "block_6  = []"
definition block_7
