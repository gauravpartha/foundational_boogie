...
definition block_4
  where
    "block_4  = [(Assume (BinOp (Var 1) Gt (Lit (LInt 0)))),(Assign 1 (BinOp (Var 1) Add (Var 0)))]"
definition outEdges
  where
    "outEdges  = [[2,1],[],[4,3],[],[]]"
...
shows "((Set.inter (set (map fst (append global_data.constants_vdecls global_data.globals_vdecls))) (set (map fst (append nested_if_before_cfg_to_dag_prog.params_vdecls nested_if_before_cfg_to_dag_prog.locals_vdecls)))) = {})"
...
shows "(\<forall> x \<tau>. (((lookup_var_ty ((append global_data.constants_vdecls global_data.globals_vdecls),(append nested_if_before_cfg_to_dag_prog.params_vdecls nested_if_before_cfg_to_dag_prog.locals_vdecls)) x) = (Some \<tau>)) \<longrightarrow> ((wf_ty 0) \<tau>)))"
