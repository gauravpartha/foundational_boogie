...
definition block_4
  where
    "block_4  = [(Assume (BinOp (Var 1) Gt (Lit (LInt 10)))),(Assign 0 (BinOp (Var 0) Mul (Lit (LInt 10))))]"
definition block_5
  where
    "block_5  = [(Assume (BinOp (Lit (LInt 10)) Ge (Var 1)))]"
definition block_6
  where
    "block_6  = []"
definition block_7
  where
    "block_7  = [(Assume (BinOp (BinOp (Var 0) Sub (Lit (LInt 10))) Gt (Lit (LInt 200)))),(Assign 1 (BinOp (Var 2) Add (Lit (LInt 7))))]"
definition block_8
  where
    "block_8  = [(Assume (BinOp (Lit (LInt 200)) Ge (BinOp (Var 0) Sub (Lit (LInt 10)))))]"
definition block_9
  where
    "block_9  = [(Assign 0 (BinOp (BinOp (Var 0) Add (Var 1)) Add (Var 2)))]"
definition block_10
  where
    "block_10  = []"
definition block_11
  where
    "block_11  = [(Assign 2 (BinOp (Var 2) Sub (Lit (LInt 2))))]"
definition outEdges
  where
    "outEdges  = [[1],[10,2],[3],[5,4],[3],[6],[7,8],[9],[9],[1],[11],[]]"
...
lemma outEdges_1:
shows "((nth (out_edges proc_body) 1) = [10,2])"
by (simp add:proc_body_def outEdges_def)
...
lemma outEdges_6:
shows "((nth (out_edges proc_body) 6) = [7,8])"
by (simp add:proc_body_def outEdges_def)

lemma outEdges_7:
shows "((nth (out_edges proc_body) 7) = [9])"
by (simp add:proc_body_def outEdges_def)

lemma outEdges_8:
shows "((nth (out_edges proc_body) 8) = [9])"
by (simp add:proc_body_def outEdges_def)

lemma outEdges_9:
shows "((nth (out_edges proc_body) 9) = [1])"
by (simp add:proc_body_def outEdges_def)

lemma outEdges_10:
shows "((nth (out_edges proc_body) 10) = [11])"
by (simp add:proc_body_def outEdges_def)
