...
imports Boogie_Lang.Semantics Boogie_Lang.Util Boogie_Lang.BackedgeElim Boogie_Lang.TypingML m_before_cfg_to_dag_prog m_before_passive_prog m_passification_proof m_vcphase_proof
begin
...
abbreviation \<Lambda>1
  where
    "\<Lambda>1  \<equiv> ((append global_data.constants_vdecls global_data.globals_vdecls),(append m_before_cfg_to_dag_prog.params_vdecls m_before_cfg_to_dag_prog.locals_vdecls))"
...
shows "(expr_all_sat A \<Lambda>1 \<Gamma> [] ns2 m_before_cfg_to_dag_prog.post)"
unfolding expr_all_sat_def m_before_cfg_to_dag_prog.post_def 
...
unfolding m_before_cfg_to_dag_prog.block_3_def m_before_passive_prog.block_1_def
...
shows "(valid_configuration A \<Lambda>1 \<Gamma> [] m_before_cfg_to_dag_prog.post m' s')"
...
apply (rule DagAssms)
apply (erule block_anon3_LoopDone)
apply assumption+
apply (rule m_before_cfg_to_dag_prog.outEdges_3)
...
"(dag_lemma_assms A \<Lambda>1 \<Gamma> [] [] [] ns1 ns2)"
shows "(dag_lemma_conclusion A M \<Lambda>1 \<Gamma> [] [(BinOp (Var 0) Le (Var 1))] m_before_passive_prog.block_2 ns2 s' True)"
using assms
apply (rule dag_rel_block_lemma_compact, simp)
unfolding m_before_cfg_to_dag_prog.block_2_def m_before_passive_prog.block_2_def
apply cfg_dag_rel_tac_single+
apply simp
apply simp
apply (erule type_safety_top_level_inv[OF Wf_Fun global_data.funcs_wf m_before_cfg_to_dag_prog.var_context_wf])
apply (simp)
apply ((tactic \<open> typing_tac @{context} [] @{thms m_before_cfg_to_dag_prog.l_i(2) m_before_cfg_to_dag_prog.l_n(2)} [] 1\<close>))
...
IH_anon3_LoopHead: "(loop_ih A M \<Lambda>1 \<Gamma> [] m_before_cfg_to_dag_prog.proc_body [0] [(BinOp (Var 0) Le (Var 1))] m_before_cfg_to_dag_prog.post ns1 s' 1 m' j)"
shows "(valid_configuration A \<Lambda>1 \<Gamma> [] m_before_cfg_to_dag_prog.post m' s')"
...
using assms
apply (rule dag_rel_block_lemma_compact, simp)
unfolding m_before_cfg_to_dag_prog.block_1_def m_before_passive_prog.block_3_def
apply cfg_dag_rel_tac_single+
apply simp
apply ((simp add:m_before_cfg_to_dag_prog.l_i(1)))
apply simp
done
...
"(dag_lemma_assms A \<Lambda>1 \<Gamma> [] [] [] ns1 ns2)"
shows "(dag_lemma_conclusion A M \<Lambda>1 \<Gamma> [] [(BinOp (Var 0) Le (Var 1))] m_before_passive_prog.block_4 ns2 s' False)"
using assms
apply (rule dag_rel_block_lemma_compact, simp)
unfolding m_before_cfg_to_dag_prog.block_0_def m_before_passive_prog.block_4_def
apply cfg_dag_rel_tac_single+
apply simp
apply simp
apply (erule type_safety_top_level_inv[OF Wf_Fun global_data.funcs_wf m_before_cfg_to_dag_prog.var_context_wf])
...
lemma cfg_block_0:
assumes 
"(\<forall> m2' s2'. ((red_cfg_multi A M \<Lambda>1 \<Gamma> [] m_before_passive_prog.proc_body ((Inl 5),(Normal ns2)) (m2',s2')) \<longrightarrow> (s2' \<noteq> Failure)))" and 
"(nstate_same_on \<Lambda>1 ns1 ns2 {})" and 
"(state_well_typed A \<Lambda>1 [] ns1)" and 
"(state_well_typed A \<Lambda>1 [] ns2)" and 
"((\<And> m2' s2'. ((red_cfg_multi A M \<Lambda>1 \<Gamma> [] m_before_passive_prog.proc_body ((Inl 4),(Normal ns2)) (m2',s2')) \<Longrightarrow> (s2' \<noteq> Failure))) \<Longrightarrow> R)"
shows "R"
using assms
apply (rule cfg_dag_empty_propagate_helper)
apply (assumption, simp)
apply ((simp add:m_before_passive_prog.outEdges_5))
by ((simp add:m_before_passive_prog.node_5 m_before_passive_prog.block_5_def))
...
"(\<And> m2' s2'. ((red_cfg_multi A M \<Lambda>1 \<Gamma> [] m_before_passive_prog.proc_body ((Inl 6),(Normal ns2)) (m2',s2')) \<Longrightarrow> (s2' \<noteq> Failure)))" and 
"(expr_all_sat A \<Lambda>1 \<Gamma> [] ns2 m_before_cfg_to_dag_prog.pres)"
shows "(valid_configuration A \<Lambda>1 \<Gamma> [] m_before_cfg_to_dag_prog.post m' s')"
apply (rule cfg_dag_helper_entry)
apply (rule m_before_passive_prog.node_6)
apply (erule assms(3))
apply (rule assms(2))
unfolding m_before_passive_prog.block_6_def
apply (rule assume_pres_normal[where ?es=m_before_cfg_to_dag_prog.pres])
apply (rule assms(4))
unfolding m_before_cfg_to_dag_prog.pres_def
apply simp
apply (rule m_before_passive_prog.outEdges_6)
apply ((simp add:m_before_passive_prog.node_5 m_before_passive_prog.block_5_def))
apply (rule m_before_passive_prog.outEdges_5)
...
abbreviation \<Lambda>0
  where
    "\<Lambda>0  \<equiv> ((append global_data.constants_vdecls global_data.globals_vdecls),(append m_before_cfg_to_dag_prog.params_vdecls m_before_cfg_to_dag_prog.locals_vdecls))"
lemma end_to_end_theorem_aux:
assumes 
Red: "(red_cfg_multi A M ((append global_data.constants_vdecls global_data.globals_vdecls),(append m_before_cfg_to_dag_prog.params_vdecls m_before_cfg_to_dag_prog.locals_vdecls)) \<Gamma> [] m_before_cfg_to_dag_prog.proc_body ((Inl 0),(Normal ns)) (m',s'))" and 
...
Precondition: "(expr_all_sat A \<Lambda>0 \<Gamma> [] ns m_before_cfg_to_dag_prog.pres)" and 
...
shows "(valid_configuration A \<Lambda>0 \<Gamma> [] m_before_cfg_to_dag_prog.post m' s')"
proof -
from Red obtain j where Aux:"(red_cfg_k_step A M ((append global_data.constants_vdecls global_data.globals_vdecls),(append m_before_cfg_to_dag_prog.params_vdecls m_before_cfg_to_dag_prog.locals_vdecls)) \<Gamma> [] m_before_cfg_to_dag_prog.proc_body ((Inl 0),(Normal ns)) j (m',s'))"
...
shows "(\<And> A. (proc_is_correct (A::(('a)absval_ty_fun)) global_data.fdecls global_data.constants_vdecls global_data.globals_vdecls global_data.axioms m_before_cfg_to_dag_prog.proc))"
apply (rule end_to_end_util[OF end_to_end_theorem_aux])
apply assumption using VC apply simp  apply assumption+
by (simp_all add: exprs_to_only_checked_spec_1 exprs_to_only_checked_spec_2 m_before_cfg_to_dag_prog.proc_def m_before_cfg_to_dag_prog.proc_body_def)
