...
definition block_2
  where
    "block_2  = [(Assume (BinOp (Var 1) Gt (Lit (LInt 0)))),(Assume (BinOp (Var 2) Eq (BinOp (Var 1) Add (Var 0))))]"
definition block_3
  where
    "block_3  = [(Assume (BinOp (Lit (LInt 0)) Ge (Var 1)))]"
definition block_4
  where
...
shows "((Set.inter (set (map fst (append nested_if_before_ast_to_cfg_prog.constants_vdecls nested_if_before_ast_to_cfg_prog.globals_vdecls))) (set (map fst (append nested_if_passive_prog.params_vdecls nested_if_passive_prog.locals_vdecls)))) = {})"
