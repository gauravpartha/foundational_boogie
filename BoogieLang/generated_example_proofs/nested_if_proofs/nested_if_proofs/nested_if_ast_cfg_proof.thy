...
proof -                                                            
  ...
  apply (rule generic_ending_block_global_rel)
           apply (rule Rel_Main_test[of outer_else])
...
proof -
  ...
  show ?thesis 
    apply (rule block_global_rel_if_successor)
           apply (rule Rel_Main_test[of outer_then])
        ...
        apply (rule assms(1))
        apply simp
    unfolding nested_if_before_cfg_to_dag_prog.post_def
       apply simp
      apply simp
     apply simp
    apply simp
...
lemma entry_block_global_rel:
  ...
  shows "(valid_configuration A \<Lambda> \<Gamma> \<Omega> nested_if_before_cfg_to_dag_prog.post reached_bb reached_cont reached_state)" 
proof -
