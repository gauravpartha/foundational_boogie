...
imports Boogie_Lang.Semantics Boogie_Lang.Util nested_loop2_before_ast_to_cfg_prog
begin
definition block_0
  where
    "block_0  = []"
...
definition block_2
  where
    "block_2  = [(Assume (BinOp (Var 6) Gt (Lit (LInt 0)))),(Assume (BinOp (Var 7) Eq (BinOp (Var 6) Sub (Lit (LInt 1))))),(Assert (BinOp (Var 7) Ge (Lit (LInt 0)))),(Assume (Lit (LBool False)))]"
definition block_3
  where
    "block_3  = [(Assume (BinOp (Var 8) Eq (BinOp (Var 4) Sub (Lit (LInt 1))))),(Assert (BinOp (Var 8) Ge (Lit (LInt 0)))),(Assume (Lit (LBool False)))]"
definition block_4
  where
    "block_4  = [(Assume (BinOp (Lit (LInt 0)) Ge (Var 6)))]"
definition block_5
  where
...
definition block_8
  where
    "block_8  = [(Assume (BinOp (Lit (LInt 0)) Ge (Var 4)))]"
definition block_9
  where
    "block_9  = [(Assume (BinOp (Var 4) Ge (Lit (LInt 0))))]"
definition block_10
  where
    "block_10  = [(Assume (Lit (LBool True))),(Assert (BinOp (Var 2) Ge (Lit (LInt 0))))]"
definition block_11
  where
    "block_11  = []"
definition block_12
...
shows "((Set.inter (set (map fst (append nested_loop2_before_ast_to_cfg_prog.constants_vdecls nested_loop2_before_ast_to_cfg_prog.globals_vdecls))) (set (map fst (append nested_loop2_passive_prog.params_vdecls nested_loop2_passive_prog.locals_vdecls)))) = {})"
