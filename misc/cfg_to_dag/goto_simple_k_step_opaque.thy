theory goto_simple_k_step_opaque
...
lemma block_loopBody_alt:
assumes 
"(red_cmd_list \<Lambda> \<Gamma> [(Assume (BinOp (Var ''i'') Lt (Var ''n''))), 
(Assign [(''i'', (BinOp (Var ''i'') Add (Var ''k'')))])] (Normal n_s) s')" and 
"((n_s ''i'') = (Some (IntV vc_i)))" and 
"((n_s ''k'') = (Some (IntV vc_k)))" and 
"((n_s ''n'') = (Some (IntV vc_n)))" and 
"(vc_passive.vc_loopBody vc_i vc_k)"
...
shows "((s' \<noteq> Failure) \<and> (\<forall> n_s'. ((s' = (Normal n_s')) \<longrightarrow> 
 (\<exists> vc_i. (((((n_s' ''i'') = (Some (IntV vc_i))) \<and> 
    ((n_s' ''k'') = (Some (IntV vc_k)))) \<and>
    ((n_s' ''n'') = (Some (IntV vc_n)))) \<and> ((vc_passive.vc_loopBody vc_i vc_k) \<and> (vc_passive.vc_afterLoop vc_i)))))))"
using assms 
apply cases
...
lemma havoc_id:
  assumes A1:"n_s x = Some v" and A2:"\<Lambda> x = Some ty" and A3:"type_of_val v = ty"
  shows "\<Lambda>, \<Gamma> \<turnstile> \<langle>Havoc x, Normal n_s\<rangle> \<rightarrow> Normal n_s"
proof -
  have "\<Lambda>, \<Gamma> \<turnstile> \<langle>Havoc x, Normal n_s\<rangle> \<rightarrow> Normal (n_s(x \<mapsto> v))"
    using A2 A3 by (rule RedHavoc)
  thus ?thesis using A1
    by (simp add: map_upd_triv) 
qed
     
lemma add_havoc:
  assumes "\<Lambda>,\<Gamma> \<turnstile> \<langle>cs, Normal n_s\<rangle> [\<rightarrow>] s'" and 
          "n_s x = Some v" and "\<Lambda> x = Some ty" and "type_of_val v = ty"
  shows "\<Lambda>,\<Gamma> \<turnstile> \<langle>(Havoc x)#cs, Normal n_s\<rangle> [\<rightarrow>] s'"
  apply rule
   apply (rule havoc_id)
  using assms 
  by auto

lemma swap_assert_assume:
  assumes A1:"\<Gamma> \<turnstile> \<langle>I, n_s\<rangle> \<Down> BoolV True" and A2:"\<Lambda>, \<Gamma> \<turnstile> \<langle>(Assert I)#cs, Normal n_s\<rangle> [\<rightarrow>] s'"
  shows "\<Lambda>, \<Gamma> \<turnstile> \<langle>(Assume I)#cs, Normal n_s\<rangle> [\<rightarrow>] s'" 
  using A2
proof cases
  case (RedCmdListCons s'')
  hence "s'' = Normal n_s"
  using A1 expr_eval_determ
  by (cases; blast)
  with \<open>\<Lambda>,\<Gamma> \<turnstile> \<langle>cs,s''\<rangle> [\<rightarrow>] s'\<close> have Acs:"\<Lambda>,\<Gamma> \<turnstile> \<langle>cs, Normal n_s\<rangle> [\<rightarrow>] s'" by simp
  show ?thesis 
  proof (rule)
    show "\<Lambda>,\<Gamma> \<turnstile> \<langle>Assume I,Normal n_s\<rangle> \<rightarrow> Normal n_s" using A1 by (rule RedAssumeOk)
  next 
    show "\<Lambda>,\<Gamma> \<turnstile> \<langle>cs,Normal n_s\<rangle> [\<rightarrow>] s'" using Acs by assumption
  qed
qed

lemma swap_assert_add_havoc:
  assumes "\<Lambda>, \<Gamma> \<turnstile> \<langle>(Assert I)#cs, Normal n_s\<rangle> [\<rightarrow>] s'" and "\<Gamma> \<turnstile> \<langle>I, n_s\<rangle> \<Down> BoolV True" and
          "n_s x = Some v" and "\<Lambda> x = Some ty" and "type_of_val v = ty"
  shows "\<Lambda>,\<Gamma> \<turnstile> \<langle>(Havoc x)#(Assume I)#cs, Normal n_s\<rangle> [\<rightarrow>] s'"
  apply (rule add_havoc, rule swap_assert_assume)
  using assms by auto
 
lemma block_loopHead_alt:
assumes 
A1:"(red_cmd_list \<Lambda> \<Gamma> [(Assert (BinOp (Var ''i'') Gt (Val (IntV 0))))] (Normal n_s) s')" and
"((n_s ''k'') = (Some (IntV vc_k)))" and 
"((n_s ''n'') = (Some (IntV vc_n)))" and
"n_s ''i'' = Some (IntV vc_i)" 
"\<Gamma> \<turnstile> \<langle>(BinOp (Var ''i'') Gt (Val (IntV 0))), n_s\<rangle> \<Down> BoolV True" and
"(vc_passive.vc_loopHead vc_k)"
shows "((s' \<noteq> Failure) \<and> (\<forall> n_s'. ((s' = (Normal n_s')) \<longrightarrow> 
  (\<exists> vc_i. (((((n_s' ''i'') = (Some (IntV vc_i))) \<and>
 ((n_s' ''k'') = (Some (IntV vc_k)))) \<and> ((n_s' ''n'') = (Some (IntV vc_n)))) \<and>
 ((vc_passive.vc_loopBody vc_i vc_k) \<and> (vc_passive.vc_afterLoop vc_i)))))))"
using A1
apply -
apply (drule swap_assert_add_havoc[where ?x="''i''"])
using assms var_assms
  apply (solves simp)+
apply (frule block_loopHead)
  using assms
  by auto
...
(* entry node *)
abbreviation Entry where "Entry \<equiv> 0"
(* loop head *)
...
definition test
  where "test f = (\<forall> x. f x = 4)"

lemma "test id \<Longrightarrow> (id x = x)"
    ...
    \<Gamma> \<turnstile> \<langle>Var ''i'' \<guillemotleft>Gt\<guillemotright> (Val (IntV 0)),ns2\<rangle> \<Down> BoolV True \<Longrightarrow> s' \<noteq> Failure"
        ...
        proof (cases)
          ...
          hence "\<Lambda>, \<Gamma> \<turnstile> \<langle>[(Assert (BinOp (Var ''i'') Gt (Val (IntV 0))))], Normal ns'\<rangle> [\<rightarrow>] s''"
            by (simp add: G_def)
          thm block_loopHead_alt
          hence A4:"s'' \<noteq> Failure \<and>
(\<forall>n_s'.
    s'' = Normal n_s' \<longrightarrow>
    (\<exists>vc_i.
        ((n_s' ''i'' = Some (IntV vc_i) \<and> n_s' ''k'' = Some (IntV vc_k)) \<and> n_s' ''n'' = Some (IntV vc_n)) \<and>
        vc_passive.vc_loopBody vc_i vc_k \<and> vc_passive.vc_afterLoop vc_i))"
            apply (rule block_loopHead_alt)
            using less.prems by auto
          show ?thesis
          proof (cases s'')
            case (Normal ns'')
            then obtain vc_i' where 
                 "ns'' ''i'' = Some (IntV vc_i')" and 
                 "ns'' ''k'' = Some (IntV vc_k)" and
                 "ns'' ''n'' = Some (IntV vc_n)"
                 "vc_passive.vc_loopBody vc_i' vc_k" and 
                 "vc_passive.vc_afterLoop vc_i'" using A4 by auto
            ...
            show ?thesis 
              ...
              using \<open>n'' = AL\<close> A3 \<open>z = (Inl n'', s'')\<close> \<open>s'' = Normal ns''\<close> 
                    \<open>ns'' ''i'' = Some (IntV vc_i')\<close> \<open>ns'' ''n'' = Some (IntV vc_n)\<close> \<open>vc_passive.vc_afterLoop vc_i'\<close>
              by simp_all
          next  
            assume "n'' = LB"
            show ?thesis
              apply (rule loopBody_aux[of k ns''])
          ...
          next
            case Failure
            then show ?thesis using A4 by simp 
          next
            case Magic
            then show ?thesis using A4 \<open>z = (Inl n'', s'')\<close> \<open>(\<Lambda>, \<Gamma>, G \<turnstile> z -n\<rightarrow>* (m', s'))\<close> magic_stays_cfg_multi by metis 
          qed
        next
          case (RedReturn cs s')
          then show ?thesis using G_def
    ...
    thm converse_rtranclpE2
(*
lemma inv:
  assumes "\<Lambda>, \<Gamma> \<turnstile> \<langle>cs, Normal ns\<rangle> [\<rightarrow>] s'" and "(\<Lambda>, \<Gamma> \<turnstile> \<langle>cs @ [Assert A], Normal ns\<rangle> [\<rightarrow>] s'' \<Longrightarrow> P(s''))
  shows "(\<Lambda>, \<Gamma> \<turnstile> \<langle>cs @ [Assert A], Normal ns\<rangle> [\<rightarrow>] s'') \<Longrightarrow> 
         (s'' \<noteq> Failure \<Longrightarrow> s' = s'' \<and> (\<forall>ns'. s' = Normal ns' \<longrightarrow> (\<Gamma> \<turnstile> \<langle>A, ns'\<rangle> \<Down> b) \<longrightarrow> (b = BoolV True)))"
  sorry
*)

lemma entry_complete_correct:
  assumes 
  A1: "\<Lambda>, \<Gamma>, G \<turnstile> (Inl Entry, Normal n_s) -n\<rightarrow>* (m', s')" and 
   "((n_s ''n'') = (Some (IntV vc_n)))" and
   "((n_s ''k'') = (Some (IntV vc_k)))" and
   "((n_s ''i'') = (Some (IntV vc_i)))"
   "vc_passive.vc_anon0"
 shows "s' \<noteq> Failure"
  using A1
proof (cases rule: converse_rtranclpE2, assumption)
  assume "(Inl 0, Normal n_s) = (m', s')"
  thus ?thesis by auto
next
  fix m'' s''
  assume B2:"\<Lambda>,\<Gamma>,G \<turnstile>(m'', s'') -n\<rightarrow>* (m', s')"
  assume "\<Lambda>,\<Gamma>,G \<turnstile> (Inl 0, Normal n_s) -n\<rightarrow> (m'', s'')"
  thus ?thesis
  proof (cases)
    case (RedNode cs n')
    hence "\<Lambda>,\<Gamma> \<turnstile> \<langle>[Assume (BinOp (Var ''k'') Ge (Val (IntV 0))),
Assign [(''i'', (BinOp (Var ''k'') Add (Val (IntV 1))))]],Normal n_s\<rangle> [\<rightarrow>] s''" using G_def
      by (metis nodeToBlocks_goto_simple.simps(4) option.inject select_convs(4))
    hence B1:"s'' \<noteq> Failure \<and> (\<forall>ns''. (s'' = Normal ns'' \<longrightarrow> 
        (\<exists>vc_i'.
            ns'' ''n'' = Some (IntV vc_n) \<and>
            ns'' ''k'' = Some (IntV vc_k) \<and>
            ns'' ''i'' = Some (IntV vc_i') \<and>
            vc_passive.vc_loopHead vc_k) \<and>
            \<Gamma> \<turnstile> \<langle>Var ''i'' \<guillemotleft>Gt\<guillemotright> (Val (IntV 0)), ns''\<rangle> \<Down> BoolV (True)))"
      apply cases
      using assms
      (* VC not opaque *)
      apply (simp only: vc_passive.vc_anon0_def)
      ...
      apply auto
      (* VC not used for showing invariant *)
      apply (rule+, simp)+
      done
    from \<open>n' \<in> out_edges G 0\<close> have "n' = LH" using G_def
      by (metis outEdges_goto_simple.simps(4) select_convs(3) singleton_iff) 
    show ?thesis
    proof (cases s'')
      case (Normal ns'')
      from this obtain vc_i' where
                 B3: "ns'' ''n'' = Some (IntV vc_n)"
                  "ns'' ''k'' = Some (IntV vc_k)"
                  "ns'' ''i'' = Some (IntV vc_i')"
                  "vc_passive.vc_loopHead vc_k" using B1 by auto
      from \<open>s'' = Normal ns''\<close> have B4: "\<Gamma> \<turnstile> \<langle>Var ''i'' \<guillemotleft>Gt\<guillemotright> (Val (IntV 0)), ns''\<rangle> \<Down> BoolV (True)"
        using B1 by simp
        
      from \<open>s'' = Normal ns''\<close> B2 \<open>n' = LH\<close> \<open>m'' = Inl n'\<close> obtain j where "\<Lambda>,\<Gamma>,G \<turnstile>(Inl LH, Normal ns'') -n\<rightarrow>^j (m', s')"
        by (meson rtranclp_imp_relpowp)
      then show ?thesis
        apply (rule loopHead_aux)
        using B3 B4 by simp_all
    next
      case Failure
      then show ?thesis using B1 by simp
    next
      case Magic
    then show ?thesis using B1 B2 magic_stays_cfg_multi by metis
  qed    
  next
  case (RedReturn cs)
  then show ?thesis using G_def
    by (metis insert_not_empty outEdges_goto_simple.simps(4) select_convs(3)) 
  qed
qed
