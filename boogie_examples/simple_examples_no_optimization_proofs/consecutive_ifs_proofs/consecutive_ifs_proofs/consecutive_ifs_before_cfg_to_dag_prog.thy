...
definition block_1
  where
    "block_1  = [(Assume (BinOp (Var 0) Gt (Lit (LInt 0)))),(Assign 0 (Lit (LInt 5)))]"
definition block_2
  where
...
definition block_5
  where
    "block_5  = [(Assign 0 (UnOp UMinus (Lit (LInt 1))))]"
definition outEdges
  where
    "outEdges  = [[1,2],[3],[3],[4,5],[],[]]"
...
lemma outEdges_5:
shows "((nth (out_edges proc_body) 5) = [])"
by (simp add:proc_body_def outEdges_def)
...
shows "(((map fst (append consecutive_ifs_before_cfg_to_dag_prog.constants_vdecls consecutive_ifs_before_cfg_to_dag_prog.globals_vdecls)) \<noteq> []) \<longrightarrow> ((Max (set (map fst (append consecutive_ifs_before_cfg_to_dag_prog.constants_vdecls consecutive_ifs_before_cfg_to_dag_prog.globals_vdecls)))) \<le> 0))"
unfolding consecutive_ifs_before_cfg_to_dag_prog.constants_vdecls_def consecutive_ifs_before_cfg_to_dag_prog.globals_vdecls_def
by simp

lemma globals_max:
shows "(\<forall> x. ((Set.member x (set (map fst (append consecutive_ifs_before_cfg_to_dag_prog.constants_vdecls consecutive_ifs_before_cfg_to_dag_prog.globals_vdecls)))) \<longrightarrow> (x \<le> 0)))"
...
shows "((Set.inter (set (map fst (append consecutive_ifs_before_cfg_to_dag_prog.constants_vdecls consecutive_ifs_before_cfg_to_dag_prog.globals_vdecls))) (set (map fst (append consecutive_ifs_before_cfg_to_dag_prog.params_vdecls consecutive_ifs_before_cfg_to_dag_prog.locals_vdecls)))) = {})"
unfolding consecutive_ifs_before_cfg_to_dag_prog.constants_vdecls_def consecutive_ifs_before_cfg_to_dag_prog.globals_vdecls_def
by simp
...
lemma consts_wf:
shows "((list_all (comp (wf_ty 0) (comp fst snd)) consecutive_ifs_before_cfg_to_dag_prog.constants_vdecls) )"
unfolding consecutive_ifs_before_cfg_to_dag_prog.constants_vdecls_def
by simp

lemma globals_wf:
shows "((list_all (comp (wf_ty 0) (comp fst snd)) consecutive_ifs_before_cfg_to_dag_prog.globals_vdecls) )"
unfolding consecutive_ifs_before_cfg_to_dag_prog.globals_vdecls_def
by simp
...
shows "(\<forall> x \<tau>. (((lookup_var_ty ((append consecutive_ifs_before_cfg_to_dag_prog.constants_vdecls consecutive_ifs_before_cfg_to_dag_prog.globals_vdecls),(append consecutive_ifs_before_cfg_to_dag_prog.params_vdecls consecutive_ifs_before_cfg_to_dag_prog.locals_vdecls)) x) = (Some \<tau>)) \<longrightarrow> ((wf_ty 0) \<tau>)))"
