...
cfgDoesntFail: "(\<And> m' s'. ((red_cfg_multi A M' \<Lambda>1 \<Gamma> [] M_before_cfg_to_dag_prog.proc_body ((Inl 10),(Normal ns1)) (m',s')) \<Longrightarrow> (s' \<noteq> Failure)))" and 
cfgSatisfiesPosts: "(\<And> m' s'. ((red_cfg_multi A M' \<Lambda>1 \<Gamma> [] M_before_cfg_to_dag_prog.proc_body ((Inl 10),(Normal ns1)) (m',s')) \<Longrightarrow> ((is_final_config (m',s')) \<Longrightarrow> (\<forall> ns_end. ((s' = (Normal ns_end)) \<longrightarrow> (expr_all_sat A \<Lambda>1 \<Gamma> [] ns_end M_before_ast_to_cfg_prog.post))))))"
...
apply (rule Rel_Invs[of M_before_ast_to_cfg_prog.bigblock_9])
...
apply (rule disjI1)
apply (rule M_before_cfg_to_dag_prog.block_10_def)
...
assumes 
VC: "(\<And> (vc_x::int) (vc_y::int) (vc_z::int) (vc_x_0::int) (vc_y_0::int) (vc_z_0::int) (vc_x_1::int) (vc_y_1::int) (vc_x_2::int) (vc_x_3::int) (vc_y_2::int) (vc_x_4::int). (vc.vc_anon0 ))"
