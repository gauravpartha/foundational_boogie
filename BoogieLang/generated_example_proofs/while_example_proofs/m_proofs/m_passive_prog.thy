...
imports Boogie_Lang.Semantics Boogie_Lang.Util m_before_passive_prog
begin
definition block_0
  where
    "block_0  = []"
definition block_1
  where
    "block_1  = [(Assume (BinOp (Var 1) Le (Var 2))),(Assert (BinOp (Var 2) Ge (Var 1)))]"
definition block_2
  where
    "block_2  = [(Assume (BinOp (Var 2) Lt (Var 1))),(Assume (BinOp (Var 3) Eq (BinOp (Var 2) Add (Lit (LInt 1))))),(Assert (BinOp (Var 3) Le (Var 1))),(Assume (Lit (LBool False)))]"
definition block_3
  where
    "block_3  = [(Assume (BinOp (Var 2) Le (Var 1)))]"
definition block_4
  where
    "block_4  = [(Assume (BinOp (Var 1) Gt (Lit (LInt 0)))),(Assert (BinOp (Lit (LInt 0)) Le (Var 1)))]"
definition block_5
  where
    "block_5  = []"
definition block_6
...
shows "((Set.inter (set (map fst (append global_data.constants_vdecls global_data.globals_vdecls))) (set (map fst (append m_passive_prog.params_vdecls m_passive_prog.locals_vdecls)))) = {})"
