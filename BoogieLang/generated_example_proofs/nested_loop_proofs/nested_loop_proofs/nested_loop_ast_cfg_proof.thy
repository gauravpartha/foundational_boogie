...
proof -
  have node3_loc: "node_to_block nested_loop_before_cfg_to_dag_prog.proc_body ! 6 = [(Assume (BinOp (Lit (LInt 0)) Ge (Var 0)))]" 
    by (simp add: nested_loop_before_cfg_to_dag_prog.block_6_def nested_loop_before_cfg_to_dag_prog.node_6)
  show ?thesis
...
lemma outer_body_bb2_global_rel:
  ...
  using assms
proof -
  have node5_loc: "node_to_block nested_loop_before_cfg_to_dag_prog.proc_body ! 5 = [(Assume (BinOp (Lit (LInt 0)) Ge (Var 1))),(Assign 0 (BinOp (Var 0) Sub (Lit (LInt 1))))]" 
    by (simp add: nested_loop_before_cfg_to_dag_prog.block_5_def nested_loop_before_cfg_to_dag_prog.node_5)
  show ?thesis 
...
lemma inner_loop_body_global_rel:
  ...
  shows "(Ast.valid_configuration A \<Lambda>1_local \<Gamma> \<Omega>  nested_loop_before_ast_cfg.post reached_bb reached_cont reached_state)"
proof -
  have node5_loc: "node_to_block nested_loop_before_cfg_to_dag_prog.proc_body ! 4 = 
                   [(Assume (BinOp (Var 1) Gt (Lit (LInt 0)))),(Assign 1 (BinOp (Var 1) Sub (Lit (LInt 1))))]" 
    by (simp add: nested_loop_before_cfg_to_dag_prog.block_4_def nested_loop_before_cfg_to_dag_prog.node_4)
  show ?thesis 
    apply (rule block_global_rel_generic)
           apply (rule Rel_Main_test[of inner_body_bb1])
             apply (simp add: inner_body_bb1_def)
