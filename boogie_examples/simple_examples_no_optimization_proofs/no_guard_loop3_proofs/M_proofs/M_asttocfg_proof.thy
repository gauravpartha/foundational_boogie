...
guardHint: "(red_expr A \<Lambda>1 \<Gamma> [] (BinOp (Var 1) Gt (Lit (LInt 10))) ns1 (BoolV False))"
...
apply (rule Rel_Invs[of M_before_ast_to_cfg_prog.bigblock_8])
...
show ?thesis 
apply (rule block_global_rel_generic)
apply (rule Rel_Invs[of M_before_ast_to_cfg_prog.bigblock_6])
...
apply (rule disjI2)
apply (rule conjI)
apply (rule M_before_cfg_to_dag_prog.block_6_def)
apply (rule conjI)
apply (simp)
apply (rule conjI)
apply (rule neg_gt)
...
apply (simp add: M_before_cfg_to_dag_prog.node_6)
...
lemma rel_M_before_ast_to_cfg_prog_bigblock_5:
assumes 
astStep: "(red_bigblock A M \<Lambda>1 \<Gamma> [] T (M_before_ast_to_cfg_prog.bigblock_5,cont0,(Normal ns1)) (reached_bb,reached_cont,reached_state))" and 
cfgBlockDoesntFail: "(\<And> s2'. ((red_cmd_list A M' \<Lambda>1 \<Gamma> [] M_before_cfg_to_dag_prog.block_5 (Normal ns1) s2') \<Longrightarrow> (s2' \<noteq> Failure)))" and 
guardHint: "(red_expr A \<Lambda>1 \<Gamma> [] (BinOp (BinOp (Var 0) Sub (Lit (LInt 10))) Gt (Lit (LInt 200))) ns1 (BoolV True))"
shows "((reached_state \<noteq> Failure) \<and> (\<forall> ns1'. ((reached_state = (Normal ns1')) \<longrightarrow> (red_cmd_list A M' \<Lambda>1 \<Gamma> [] M_before_cfg_to_dag_prog.block_5 (Normal ns1) (Normal ns1')))))"
...
apply ((simp add: assms(3) M_before_ast_to_cfg_prog.bigblock_5_def)+)
done
...
cfgDoesntFail: "(\<And> m' s'. ((red_cfg_multi A M' \<Lambda>1 \<Gamma> [] M_before_cfg_to_dag_prog.proc_body ((Inl 5),(Normal ns1)) (m',s')) \<Longrightarrow> (s' \<noteq> Failure)))" and 
cfgSatisfiesPosts: "(\<And> m' s'. ((red_cfg_multi A M' \<Lambda>1 \<Gamma> [] M_before_cfg_to_dag_prog.proc_body ((Inl 5),(Normal ns1)) (m',s')) \<Longrightarrow> ((is_final_config (m',s')) \<Longrightarrow> (\<forall> ns_end. ((s' = (Normal ns_end)) \<longrightarrow> (expr_all_sat A \<Lambda>1 \<Gamma> [] ns_end M_before_ast_to_cfg_prog.post))))))" and 
...
apply (rule disjI1)
apply (rule conjI)
apply (rule M_before_cfg_to_dag_prog.block_5_def)
apply (rule conjI)
apply (simp)
apply (rule guardHint)
apply (rule cfgDoesntFail)
...
apply (rule rel_M_before_ast_to_cfg_prog_bigblock_5)
apply assumption
apply (simp)
apply (rule guardHint)
apply ((erule allE[where x=3])+)
...
apply (simp add: M_before_ast_to_cfg_prog.bigblock_4_def M_before_cfg_to_dag_prog.block_4_def)
apply ((simp add: M_before_cfg_to_dag_prog.block_4_def)+)
apply (rule astStep)
apply (rule cfgBlockDoesntFail)
apply ((simp add: M_before_cfg_to_dag_prog.block_4_def M_before_ast_to_cfg_prog.bigblock_4_def)+)
done
...
apply (simp add: cont_4_def M_before_ast_to_cfg_prog.bigblock_5_def cont_5_def )
apply blast+
...
apply (simp add: cont_4_def M_before_ast_to_cfg_prog.bigblock_6_def cont_6_def )
apply blast+
apply (rule loop_IH_prove)
apply (rule loop_IH_apply)
apply (rule inductionHypothesis)
...
guardHint: "(personal_guard = (None ))" and 
inductionHypothesis: "(loop_IH j A M M' \<Lambda>1 \<Gamma> [] T M_before_ast_to_cfg_prog.bigblock_1 cont_1 M_before_cfg_to_dag_prog.proc_body 1 M_before_ast_to_cfg_prog.post reached_bb reached_cont reached_state)"
shows "(Ast.valid_configuration A \<Lambda>1 \<Gamma> [] M_before_ast_to_cfg_prog.post reached_bb reached_cont reached_state)"
using assms
...
apply (rule astTrace)
apply (rule Rel_Main_test[of M_before_ast_to_cfg_prog.bigblock_2])
apply (simp add: M_before_ast_to_cfg_prog.bigblock_2_def M_before_cfg_to_dag_prog.block_2_def)
apply (simp add: M_before_ast_to_cfg_prog.bigblock_2_def M_before_cfg_to_dag_prog.block_2_def)
apply (simp add: M_before_ast_to_cfg_prog.bigblock_2_def M_before_cfg_to_dag_prog.block_2_def)
apply (simp add: M_before_cfg_to_dag_prog.block_2_def)
...
apply (simp add: cont_1_def M_before_ast_to_cfg_prog.bigblock_2_def cont_2_def M_before_ast_to_cfg_prog.bigblock_7_def cont_7_def )
apply ((blast)+)
apply (rule loop_IH_prove)
apply (rule less.IH)
apply (erule strictly_smaller_helper2)
...
assumes 
"(rtranclp (red_bigblock A M \<Lambda> \<Gamma> \<Omega> T) (init_ast [bigblock_0,bigblock_8] ns) (reached_bb,reached_cont,reached_state))"
...
using assms
by (simp add: cont_0_def cont_8_def )

lemma end_to_end_theorem_ast:
assumes 
VC: "(\<And> (vc_x::int) (vc_y::int) (vc_z::int) (vc_x_0::int) (vc_y_0::int) (vc_z_0::int) (vc_y_1::int) (vc_x_1::int) (vc_x_2::int) (vc_y_2::int) (vc_x_3::int) (vc_x_4::int) (vc_y_3::int). (vc.vc_anon0 ))"
