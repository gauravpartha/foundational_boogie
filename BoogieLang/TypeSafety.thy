...
lemma unop_type_correct: "\<lbrakk> unop_type uop = (arg_ty, ret_ty); type_of_val A v' = Some (TPrim arg_ty);
                         (unop_eval_val uop v') = Some v  \<rbrakk> \<Longrightarrow>
                       type_of_val A v = Some (TPrim ret_ty)"
  by (cases uop; rule lit_val_elim[where v=v']; auto)

lemma unop_progress: "\<lbrakk> unop_type uop = (arg_ty, ret_ty); type_of_val A v' = Some (TPrim arg_ty) \<rbrakk> \<Longrightarrow>
                       \<exists>v. (unop_eval_val uop v') = Some v "
  by (cases uop; rule lit_val_elim[where v=v']; auto)

lemma binop_type_correct: 
 "\<lbrakk> binop_type bop = Some ((left_ty,right_ty), ret_ty); 
    type_of_val  A v1 = Some (TPrim left_ty); type_of_val A v2 = Some (TPrim right_ty); 
...
lemma binop_progress:
 "\<lbrakk> binop_type bop = Some ((left_ty,right_ty), ret_ty); 
    type_of_val  A v1 = Some (TPrim left_ty); type_of_val A v2 = Some (TPrim right_ty)\<rbrakk> \<Longrightarrow>
    \<exists>v. (binop_eval_val bop v1 v2) = Some v "
  by (cases bop; rule lit_val_elim[where v=v1]; rule lit_val_elim[where v=v2]; auto)

lemma binop_poly_type_correct:
 "\<lbrakk> binop_poly_type bop; binop_eval_val bop v1 v2 = Some v \<rbrakk> \<Longrightarrow> type_of_val A v = Some (TPrim TBool)"
  by (cases bop; rule lit_val_elim[where v=v1]; rule lit_val_elim[where v=v2]; auto)
...
theorem preservation:
  assumes 
          ...
          "\<forall> k \<tau>'. (\<Delta> k = Some \<tau>') \<longrightarrow> (\<exists>v. (n_s k = Some v) \<and> type_of_val A v = Some (instantiate \<Omega> \<tau>'))" and
          Wf_\<Gamma>:"fun_interp_wf A F \<Gamma>" and
          Wf_F:"list_all (wf_fdecl \<circ> snd) F" 
  shows "F, \<Delta> \<turnstile> e : \<tau> \<Longrightarrow> wf_expr (length \<Omega>) e \<Longrightarrow> A,(F,\<Gamma>),\<Omega> \<turnstile> \<langle>e,n_s\<rangle> \<Down> v \<Longrightarrow> type_of_val A v = Some (instantiate \<Omega> \<tau>)" and 
        "F, \<Delta> \<turnstile> es [:] ts \<Longrightarrow> list_all (wf_expr (length \<Omega>)) es \<Longrightarrow> A,(F,\<Gamma>),\<Omega> \<turnstile> \<langle>es,n_s\<rangle> [\<Down>] vs \<Longrightarrow>
          map (type_of_val A) vs = map Some (map (instantiate \<Omega>) ts)"
  using assms
...
next
  ...
  moreover from this have "type_of_val A v' = Some (TPrim arg_ty)" using TypUnOp by auto
...
next
  ...
  from this obtain v1 v2 where 
    ...
    by auto
  moreover from this have T1:"type_of_val A v1 = Some (TPrim left_ty)" and 
    T2:"type_of_val A v2 = Some (TPrim right_ty)" using TypBinOpMono by auto
...
next
  ...
  have
   "map (type_of_val A) vargs = map Some (map (instantiate \<Omega>) (map (msubstT_opt ty_params) args_ty))"
  ...
  hence "type_of_val A v = Some (instantiate (map (instantiate \<Omega>) ty_params) ret_ty)"
...
next
  ...
  with TypListCons have A1:"type_of_val A (hd vs) = Some ((instantiate \<Omega>) ty)" and
                        A2:"map (type_of_val A) (tl vs) = map Some (map (instantiate \<Omega>) tys)"
    by auto
...
next
  ...
  hence "type_of_val A v' = Some (TPrim arg_ty)" 
...
next
  ...
  from this obtain v1 v2 where RedLeft:"A,(F, \<Gamma>),\<Omega> \<turnstile> \<langle>e1,n_s\<rangle> \<Down> v1" and  RedRight:"A,(F, \<Gamma>),\<Omega> \<turnstile> \<langle>e2,n_s\<rangle> \<Down> v2"
    by fastforce
  moreover from RedLeft have "type_of_val A v1 = Some (TPrim left_ty)"
    ...
    by fastforce
  moreover from RedRight have "type_of_val A v2 = Some (TPrim right_ty)"
    using TypBinOpMono.IH TypBinOpMono.prems preservation(1)[OF \<open>list_all closed \<Omega>\<close> TypBinOpMono.prems(5) Wf_\<Gamma> Wf_F]
    by fastforce
...
next
  ...
  hence "map (type_of_val A) vargs = map Some (map (instantiate (map (instantiate \<Omega>) ty_params)) args_ty)"     
...
next
  case (TypForall \<Delta> ty e)
  have RedBody:"\<And>w. type_of_val A w = Some (instantiate \<Omega> ty) \<Longrightarrow> \<exists>v'. A, (F, \<Gamma>), \<Omega> \<turnstile> \<langle>e, ext_env n_s w\<rangle> \<Down> v'"
  ...
  have EnvCorres:"\<And>w. type_of_val A w = Some (instantiate \<Omega> ty) \<Longrightarrow> \<forall>k \<tau>'. (ext_env \<Delta> ty) k = Some \<tau>' \<longrightarrow> 
                (\<exists>v. (ext_env n_s w) k = Some v \<and> type_of_val A v = Some (instantiate \<Omega> \<tau>'))"
    using TypForall.prems(5)
    by simp
  
  have RedBodyTy:"\<And>w v'. type_of_val A w = Some (instantiate \<Omega> ty) \<Longrightarrow> A, (F, \<Gamma>), \<Omega> \<turnstile> \<langle>e, ext_env n_s w\<rangle> \<Down> v' \<Longrightarrow>
        type_of_val A v' = Some (TPrim TBool)"
    using preservation(1)[OF \<open>list_all closed \<Omega>\<close> EnvCorres Wf_\<Gamma> Wf_F]
          TypForall.IH(1) TypForall.prems(2)
    by (metis instantiate.simps(2) wf_expr.simps(6)) 
  show ?case
  proof (cases "\<forall> w. type_of_val A w = Some (instantiate \<Omega> ty) \<longrightarrow> A, (F, \<Gamma>), \<Omega> \<turnstile> \<langle>e, ext_env n_s w\<rangle> \<Down> LitV (LBool True)")
    case True
    show ?thesis
  ...
  next
    ...
    from this obtain w where
       "type_of_val A w = Some (instantiate \<Omega> ty)" and "\<not> (A,(F, \<Gamma>),\<Omega> \<turnstile> \<langle>e, ext_env n_s w\<rangle> \<Down> LitV (LBool True))"
      by auto
    moreover from this RedBody RedBodyTy obtain w' where "A,(F, \<Gamma>),\<Omega> \<turnstile> \<langle>e, ext_env n_s w\<rangle> \<Down> w'"
      and "type_of_val A w' = Some (TPrim (TBool))"
    by fastforce
...
next
  ...
  have RedBodyTy:"\<And>w v'. type_of_val A w = Some (instantiate \<Omega> ty) \<Longrightarrow> A, (F, \<Gamma>), \<Omega> \<turnstile> \<langle>e, ext_env n_s w\<rangle> \<Down> v' \<Longrightarrow>
        type_of_val A v' = Some (TPrim TBool)"
    using preservation(1)[OF \<open>list_all closed \<Omega>\<close> EnvCorres Wf_\<Gamma> Wf_F]
          TypExists.IH(1) TypExists.prems(2) by (metis instantiate.simps(2) wf_expr.simps(7)) 
  show ?case
  proof (cases "\<forall> w. type_of_val A w = Some (instantiate \<Omega> ty) \<longrightarrow> A, (F, \<Gamma>), \<Omega> \<turnstile> \<langle>e, ext_env n_s w\<rangle> \<Down> LitV (LBool False)")
    case True
    show ?thesis
  ...
  next
    case False
    from this obtain w where
       "type_of_val A w = Some (instantiate \<Omega> ty)" and "\<not> (A,(F, \<Gamma>),\<Omega> \<turnstile> \<langle>e, ext_env n_s w\<rangle> \<Down> LitV (LBool False))"
      by auto
    moreover from this RedBody RedBodyTy obtain w' where "A,(F, \<Gamma>),\<Omega> \<turnstile> \<langle>e, ext_env n_s w\<rangle> \<Down> w'"
      and "type_of_val A w' = Some (TPrim (TBool))"
    by fastforce
...
next
  ...
  have RedBodyTy:"\<And>t v. closed t \<Longrightarrow> A,(F, \<Gamma>), t#\<Omega> \<turnstile> \<langle>e, n_s\<rangle> \<Down> v \<Longrightarrow> type_of_val A v = Some (instantiate (t#\<Omega>) (TPrim TBool))"
  ...
  next
    ...
    moreover from this RedBody RedBodyTy obtain w' where "A,(F, \<Gamma>),(t#\<Omega>) \<turnstile> \<langle>e, n_s\<rangle> \<Down> w'"
      and "type_of_val A w' = Some (TPrim (TBool))" by fastforce
    ultimately show ?thesis
