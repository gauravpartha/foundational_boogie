theory assert_false_in_if_passive_prog
imports Boogie_Lang.Semantics Boogie_Lang.Util assert_false_in_if_before_ast_to_cfg_prog
begin
definition block_0
  where
    "block_0  = []"
definition block_1
  where
    "block_1  = [(Assume (BinOp (Var 1) Neq (Lit (LInt 8)))),(Assert (Lit (LBool False)))]"
definition block_2
  where
...
shows "(((map fst (append assert_false_in_if_passive_prog.params_vdecls assert_false_in_if_passive_prog.locals_vdecls)) \<noteq> []) \<longrightarrow> ((Min (set (map fst (append assert_false_in_if_passive_prog.params_vdecls assert_false_in_if_passive_prog.locals_vdecls)))) \<ge> 0))"
unfolding assert_false_in_if_passive_prog.params_vdecls_def assert_false_in_if_passive_prog.locals_vdecls_def
by simp

lemma locals_min:
shows "(\<forall> x. ((Set.member x (set (map fst (append assert_false_in_if_passive_prog.params_vdecls assert_false_in_if_passive_prog.locals_vdecls)))) \<longrightarrow> (x \<ge> 0)))"
using locals_min_aux helper_min
by blast

lemma globals_locals_disj:
shows "((Set.inter (set (map fst (append assert_false_in_if_before_ast_to_cfg_prog.constants_vdecls assert_false_in_if_before_ast_to_cfg_prog.globals_vdecls))) (set (map fst (append assert_false_in_if_passive_prog.params_vdecls assert_false_in_if_passive_prog.locals_vdecls)))) = {})"
...
lemma m_x:
shows "((map_of (append assert_false_in_if_passive_prog.params_vdecls assert_false_in_if_passive_prog.locals_vdecls) 0) = (Some ((TPrim TInt),(None ))))"
by (simp add:params_vdecls_def locals_vdecls_def)

lemma m_x_0:
shows "((map_of (append assert_false_in_if_passive_prog.params_vdecls assert_false_in_if_passive_prog.locals_vdecls) 1) = (Some ((TPrim TInt),(None ))))"
by (simp add:params_vdecls_def locals_vdecls_def)
