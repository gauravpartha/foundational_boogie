theory q_before_cfg_to_dag_prog
...
shows "(((map fst (append q_before_cfg_to_dag_prog.params_vdecls q_before_cfg_to_dag_prog.locals_vdecls)) \<noteq> []) \<longrightarrow> ((Min (set (map fst (append q_before_cfg_to_dag_prog.params_vdecls q_before_cfg_to_dag_prog.locals_vdecls)))) \<ge> 0))"
unfolding q_before_cfg_to_dag_prog.params_vdecls_def q_before_cfg_to_dag_prog.locals_vdecls_def
by simp

lemma locals_min:
shows "(\<forall> x. ((Set.member x (set (map fst (append q_before_cfg_to_dag_prog.params_vdecls q_before_cfg_to_dag_prog.locals_vdecls)))) \<longrightarrow> (x \<ge> 0)))"
using locals_min_aux helper_min
by blast

lemma globals_locals_disj:
shows "((Set.inter (set (map fst (append global_data.constants_vdecls global_data.globals_vdecls))) (set (map fst (append q_before_cfg_to_dag_prog.params_vdecls q_before_cfg_to_dag_prog.locals_vdecls)))) = {})"
...
lemma params_wf:
shows "((list_all (comp (wf_ty 0) (comp fst snd)) q_before_cfg_to_dag_prog.params_vdecls) )"
unfolding q_before_cfg_to_dag_prog.params_vdecls_def
by simp

lemma locals_wf:
shows "((list_all (comp (wf_ty 0) (comp fst snd)) q_before_cfg_to_dag_prog.locals_vdecls) )"
unfolding q_before_cfg_to_dag_prog.locals_vdecls_def
by simp

lemma var_context_wf:
shows "(\<forall> x \<tau>. (((lookup_var_ty ((append global_data.constants_vdecls global_data.globals_vdecls),(append q_before_cfg_to_dag_prog.params_vdecls q_before_cfg_to_dag_prog.locals_vdecls)) x) = (Some \<tau>)) \<longrightarrow> ((wf_ty 0) \<tau>)))"
apply (rule lookup_ty_pred_2)
by ((simp_all add:consts_wf globals_wf params_wf locals_wf))

lemma m_x:
shows "((map_of (append q_before_cfg_to_dag_prog.params_vdecls q_before_cfg_to_dag_prog.locals_vdecls) 0) = (Some ((TPrim TInt),(None ))))"
by (simp add:params_vdecls_def locals_vdecls_def)

lemma l_x:
shows "((lookup_var_decl ((append global_data.constants_vdecls global_data.globals_vdecls),(append q_before_cfg_to_dag_prog.params_vdecls q_before_cfg_to_dag_prog.locals_vdecls)) 0) = (Some ((TPrim TInt),(None ))))" and "((lookup_var_ty ((append global_data.constants_vdecls global_data.globals_vdecls),(append q_before_cfg_to_dag_prog.params_vdecls q_before_cfg_to_dag_prog.locals_vdecls)) 0) = (Some (TPrim TInt)))"
