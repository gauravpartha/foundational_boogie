theory consecutive_ifs_passive_prog
imports Boogie_Lang.Semantics Boogie_Lang.Util consecutive_ifs_before_ast_to_cfg_prog
begin
definition block_0
  where
    "block_0  = []"
...
definition block_2
  where
    "block_2  = [(Assume (BinOp (Var 2) Eq (UnOp UMinus (Lit (LInt 1)))))]"
definition block_3
  where
    "block_3  = []"
definition block_4
...
shows "(((map fst (append consecutive_ifs_passive_prog.params_vdecls consecutive_ifs_passive_prog.locals_vdecls)) \<noteq> []) \<longrightarrow> ((Min (set (map fst (append consecutive_ifs_passive_prog.params_vdecls consecutive_ifs_passive_prog.locals_vdecls)))) \<ge> 0))"
unfolding consecutive_ifs_passive_prog.params_vdecls_def consecutive_ifs_passive_prog.locals_vdecls_def
by simp

lemma locals_min:
shows "(\<forall> x. ((Set.member x (set (map fst (append consecutive_ifs_passive_prog.params_vdecls consecutive_ifs_passive_prog.locals_vdecls)))) \<longrightarrow> (x \<ge> 0)))"
using locals_min_aux helper_min
by blast

lemma globals_locals_disj:
shows "((Set.inter (set (map fst (append consecutive_ifs_before_ast_to_cfg_prog.constants_vdecls consecutive_ifs_before_ast_to_cfg_prog.globals_vdecls))) (set (map fst (append consecutive_ifs_passive_prog.params_vdecls consecutive_ifs_passive_prog.locals_vdecls)))) = {})"
