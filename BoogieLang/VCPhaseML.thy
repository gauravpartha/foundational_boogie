...
(* here do not use the hack, should work for cases where directly call this and only have a single subgoal *)
fun vc_expr_rel_select_final_tac ctxt red_expr_tac assms (t,i) =
  case (Logic.strip_assums_concl t) of
...
fun b_vc_expr_rel_final_tac ctxt assms forall_and_exists_thm_tuple del_thms =
  REPEAT o SUBGOAL (vc_expr_rel_select_final_tac ctxt  (fn ctxt => fn assms => vc_expr_rel_red_tac ctxt assms forall_and_exists_thm_tuple del_thms) assms )
...
fun b_assume_base_tac ctxt inst_thm vc_elim expr_hint global_assms forall_and_exists_thm_tuple i =
...
fun b_assume_foc_tac global_assms forall_and_exists_thm_tuple vc_elim expr_hint i (focus: Subgoal.focus) =
let
...
fun b_assume_simple_tac ctxt global_assms forall_and_exists_thm_tuple vc_elim expr_hint n_assms_rem =
  (Subgoal.FOCUS (b_assume_foc_tac global_assms forall_and_exists_thm_tuple vc_elim expr_hint 1) ctxt 1) THEN   
   remove_n_assms n_assms_rem 1


fun b_assume_conjr_tac ctxt global_assms forall_and_exists_thm_tuple expr_hint n =
  let val vc_elim = 
    fn (vc) => @{thm imp_conj_elim} OF [(apply_thm_n_times vc @{thm imp_conj_imp} (n-1))]
  in
    (b_assume_simple_tac ctxt global_assms forall_and_exists_thm_tuple vc_elim expr_hint 3)
  end
\<close>
...
fun b_assert_base_tac ctxt inst_thm vc_expr global_assms forall_and_exists_thm_tuple ehint i =
  ...
  case vc_expr_new_option of 
     SOME vc_expr_new => b_prove_assert_expr_tac ctxt vc_expr_new global_assms forall_and_exists_thm_tuple i
  |  NONE => no_tac
  end)
 
fun b_assert_conj_foc_tac global_assms forall_and_exists_thm_tuple vc_elim ehint i (focus: Subgoal.focus) =
let 
  val (red :: vc :: _) = #prems(focus)
  val ctxt = #context(focus)
in
...
fun b_assert_conj_tac ctxt global_assms forall_and_exists_thm_tuple vc_elim ehint =
  (Subgoal.FOCUS (b_assert_conj_foc_tac global_assms forall_and_exists_thm_tuple vc_elim ehint 1) ctxt 1)  THEN
  eresolve0_tac [thin_rl] 1 THEN
  eresolve0_tac [thin_rl] 1

fun b_assert_no_conj_foc_tac global_assms forall_and_exists_thm_tuple ehint i (focus: Subgoal.focus) =
let 
  val (red :: vc :: _) = #prems(focus) 
  val ctxt = #context(focus)
in
