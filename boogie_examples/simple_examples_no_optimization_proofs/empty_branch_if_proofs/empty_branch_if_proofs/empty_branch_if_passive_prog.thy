theory empty_branch_if_passive_prog
imports Boogie_Lang.Semantics Boogie_Lang.Util empty_branch_if_before_ast_to_cfg_prog
begin
definition block_0
  where
    "block_0  = [(Assert (BinOp (Var 2) Gt (Lit (LInt 5))))]"
definition block_1
  where
    "block_1  = [(Assume (BinOp (Var 1) Gt (Lit (LInt 5)))),(Assume (BinOp (Var 2) Eq (Var 1)))]"
definition block_2
  where
    "block_2  = [(Assume (BinOp (Lit (LInt 5)) Ge (Var 1))),(Assume (BinOp (Var 2) Eq (Lit (LInt 6))))]"
definition block_3
  where
    "block_3  = []"
definition block_4
...
shows "(((map fst (append empty_branch_if_passive_prog.params_vdecls empty_branch_if_passive_prog.locals_vdecls)) \<noteq> []) \<longrightarrow> ((Min (set (map fst (append empty_branch_if_passive_prog.params_vdecls empty_branch_if_passive_prog.locals_vdecls)))) \<ge> 0))"
unfolding empty_branch_if_passive_prog.params_vdecls_def empty_branch_if_passive_prog.locals_vdecls_def
by simp

lemma locals_min:
shows "(\<forall> x. ((Set.member x (set (map fst (append empty_branch_if_passive_prog.params_vdecls empty_branch_if_passive_prog.locals_vdecls)))) \<longrightarrow> (x \<ge> 0)))"
using locals_min_aux helper_min
by blast

lemma globals_locals_disj:
shows "((Set.inter (set (map fst (append empty_branch_if_before_ast_to_cfg_prog.constants_vdecls empty_branch_if_before_ast_to_cfg_prog.globals_vdecls))) (set (map fst (append empty_branch_if_passive_prog.params_vdecls empty_branch_if_passive_prog.locals_vdecls)))) = {})"
...
lemma m_x:
shows "((map_of (append empty_branch_if_passive_prog.params_vdecls empty_branch_if_passive_prog.locals_vdecls) 0) = (Some ((TPrim TInt),(None ))))"
by (simp add:params_vdecls_def locals_vdecls_def)

lemma m_x_0:
shows "((map_of (append empty_branch_if_passive_prog.params_vdecls empty_branch_if_passive_prog.locals_vdecls) 1) = (Some ((TPrim TInt),(None ))))"
by (simp add:params_vdecls_def locals_vdecls_def)

lemma m_x_1:
shows "((map_of (append empty_branch_if_passive_prog.params_vdecls empty_branch_if_passive_prog.locals_vdecls) 2) = (Some ((TPrim TInt),(None ))))"
by (simp add:params_vdecls_def locals_vdecls_def)
