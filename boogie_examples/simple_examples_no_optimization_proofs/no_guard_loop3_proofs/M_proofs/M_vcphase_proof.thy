...
fixes A :: "(('a)absval_ty_fun)" and \<Lambda> :: "(var_context)" and \<Gamma> :: "(('a)fun_interp)" and n_s :: "(('a)nstate)" and vc_x :: "int" and vc_y :: "int" and vc_z :: "int" and vc_x_0 :: "int" and vc_y_0 :: "int" and vc_z_0 :: "int" and vc_y_1 :: "int" and vc_x_1 :: "int" and vc_x_2 :: "int" and vc_y_2 :: "int" and vc_x_3 :: "int" and vc_x_4 :: "int" and vc_y_3 :: "int"
assumes 
...
G9: "((lookup_var \<Lambda> n_s 9) = (Some (IntV vc_y_2)))" and 
G10: "((lookup_var \<Lambda> n_s 10) = (Some (IntV vc_x_3)))" and 
...
shows "((red_cmd_list A M \<Lambda> \<Gamma> \<Omega> M_passive_prog.block_1 (Normal n_s) s') \<Longrightarrow> ((vc.vc_anon0 ) \<Longrightarrow> ((s' \<noteq> Failure) \<and> (\<forall> n_s'. ((s' = (Normal n_s')) \<longrightarrow> (n_s' = n_s))))))"
...
shows "((red_cmd_list A M \<Lambda> \<Gamma> \<Omega> M_passive_prog.block_4 (Normal n_s) s') \<Longrightarrow> ((vc.vc_anon0 ) \<Longrightarrow> ((s' \<noteq> Failure) \<and> (\<forall> n_s'. ((s' = (Normal n_s')) \<longrightarrow> ((n_s' = n_s) \<and> ((vc.vc_anon0 ) \<and> (vc.vc_anon0 ))))))))"
...
shows "((red_cmd_list A M \<Lambda> \<Gamma> \<Omega> M_passive_prog.block_5 (Normal n_s) s') \<Longrightarrow> ((vc.vc_anon0 ) \<Longrightarrow> (s' = Magic)))"
unfolding M_passive_prog.block_5_def vc.vc_anon0_def
...
assumes 
"(red_cmd_list A M \<Lambda> \<Gamma> \<Omega> M_passive_prog.block_6 (Normal n_s) s')" and 
"(vc.vc_anon0 )"
shows "((s' \<noteq> Failure) \<and> (\<forall> n_s'. ((s' = (Normal n_s')) \<longrightarrow> ((n_s' = n_s) \<and> (vc.vc_anon0 )))))"
using assms
unfolding M_passive_prog.block_6_def
apply cases
by auto
...
shows "((red_cmd_list A M \<Lambda> \<Gamma> \<Omega> M_passive_prog.block_12 (Normal n_s) s') \<Longrightarrow> ((vc.vc_anon0 ) \<Longrightarrow> ((s' \<noteq> Failure) \<and> (\<forall> n_s'. ((s' = (Normal n_s')) \<longrightarrow> ((n_s' = n_s) \<and> ((vc.vc_anon0 ) \<and> (vc.vc_anon0 ))))))))"
...
shows "(s' \<noteq> Failure)"
apply (rule converse_rtranclpE2[OF assms(1)], fastforce)
apply (rule red_cfg_multi_backwards_step_magic[OF assms(1) M_passive_prog.node_5])
by (erule block_anon7_LoopDone[OF _ assms(2)])

lemma cfg_block_anon7_LoopDoneAA0:
assumes 
...
apply (erule member_elim, simp)
apply (erule cfg_block_anon7_LoopDoneAA0, simp?)
apply (erule member_elim, simp)
...
apply (simp only:state_typ_wf_def)
apply (erule allE, erule allE, erule impE, rule map_of_lookup_vdecls_ty[OF M_passive_prog.m_y_3])
apply (subst lookup_var_local[OF M_passive_prog.m_y_3])+
by (fastforce dest: tint_intv tbool_boolv)
