...
lemma block2_global_rel:
  ...
  and cfg_satisfies_post: "\<And>m' s'.
                           (A,M,\<Lambda>,\<Gamma>,\<Omega>,empty_branch_if_before_cfg_to_dag_prog.proc_body \<turnstile>(Inl 2, Normal ns1) -n\<rightarrow>* (m', s')) \<Longrightarrow>
                           is_final_config (m', s') \<Longrightarrow> (\<forall>ns_end. s' = Normal ns_end \<longrightarrow> list_all (expr_sat A \<Lambda> \<Gamma> \<Omega> ns_end) empty_branch_if_before_ast_cfg.post)"
  shows "(Ast.valid_configuration A \<Lambda> \<Gamma> \<Omega> empty_branch_if_before_ast_cfg.post reached_bb reached_cont reached_state)" 
  using assms
proof -
  show ?thesis
...
lemma block_then_global_rel:
  ...
  and cfg_satisfies_post: "\<And>m' s'.
                           (A,M,\<Lambda>,\<Gamma>,\<Omega>,empty_branch_if_before_cfg_to_dag_prog.proc_body \<turnstile>(Inl 3, Normal ns1) -n\<rightarrow>* (m', s')) \<Longrightarrow>
                           is_final_config (m', s') \<Longrightarrow> (\<forall>ns_end. s' = Normal ns_end \<longrightarrow> list_all (expr_sat A \<Lambda> \<Gamma> \<Omega> ns_end) empty_branch_if_before_ast_cfg.post)"
...
lemma block_else_global_rel:
  ...
  and cfg_satisfies_post: "\<And>m' s'.
                           (A,M,\<Lambda>,\<Gamma>,\<Omega>,empty_branch_if_before_cfg_to_dag_prog.proc_body \<turnstile>(Inl 1, Normal ns1) -n\<rightarrow>* (m', s')) \<Longrightarrow>
                           is_final_config (m', s') \<Longrightarrow> (\<forall>ns_end. s' = Normal ns_end \<longrightarrow> list_all (expr_sat A \<Lambda> \<Gamma> \<Omega> ns_end) empty_branch_if_before_ast_cfg.post)"
  and trace_is_possible: "A,\<Lambda>,\<Gamma>,\<Omega> \<turnstile> \<langle>(BinOp (Var 0) Gt (Lit (LInt 5))), ns1\<rangle> \<Down> LitV (LBool False)"
  shows "(Ast.valid_configuration A \<Lambda> \<Gamma> \<Omega> empty_branch_if_before_ast_cfg.post reached_bb reached_cont reached_state)" 
proof -
...
lemma block0_global_rel:
  ...
  and cfg_satisfies_post: "\<And>m' s'.
                           (A,M,\<Lambda>,\<Gamma>,\<Omega>,empty_branch_if_before_cfg_to_dag_prog.proc_body \<turnstile>(Inl 0, Normal ns1) -n\<rightarrow>* (m', s')) \<Longrightarrow>
                           is_final_config (m', s') \<Longrightarrow> (\<forall>ns_end. s' = Normal ns_end \<longrightarrow> list_all (expr_sat A \<Lambda> \<Gamma> \<Omega> ns_end) empty_branch_if_before_ast_cfg.post)"
  ...
  shows "(valid_configuration A \<Lambda> \<Gamma> \<Omega> empty_branch_if_before_ast_cfg.post reached_bb reached_cont reached_state)" 
proof -
  show ?thesis 
