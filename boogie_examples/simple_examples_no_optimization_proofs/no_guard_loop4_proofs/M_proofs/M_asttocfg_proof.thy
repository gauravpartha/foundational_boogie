...
cfgSatisfiesPosts: "(\<And> m' s'. ((red_cfg_multi A M' \<Lambda>1 \<Gamma> [] M_before_cfg_to_dag_prog.proc_body ((Inl 9),(Normal ns1)) (m',s')) \<Longrightarrow> ((is_final_config (m',s')) \<Longrightarrow> (\<forall> ns_end. ((s' = (Normal ns_end)) \<longrightarrow> (expr_all_sat A \<Lambda>1 \<Gamma> [] ns_end M_before_ast_to_cfg_prog.post))))))"
...
cfgBlockDoesntFail: "(\<And> s2'. ((red_cmd_list A M' \<Lambda>1 \<Gamma> [] M_before_cfg_to_dag_prog.block_2 (Normal ns1) s2') \<Longrightarrow> (s2' \<noteq> Failure)))"
...
apply ((simp add: M_before_cfg_to_dag_prog.block_2_def M_before_ast_to_cfg_prog.bigblock_2_def)+)
done
...
apply (simp add: cont_1_def M_before_ast_to_cfg_prog.bigblock_2_def cont_2_def M_before_ast_to_cfg_prog.bigblock_5_def cont_5_def )
apply ((blast)+)
apply (rule loop_IH_prove)
apply (rule less.IH)
apply (erule strictly_smaller_helper2)
...
assumes 
VC: "(\<And> (vc_x::int) (vc_y::int) (vc_z::int) (vc_x_0::int) (vc_y_0::int) (vc_z_0::int) (vc_y_1::int) (vc_x_1::int) (vc_x_2::int) (vc_x_3::int) (vc_x_4::int) (vc_y_2::int). (vc.vc_anon0 ))"
