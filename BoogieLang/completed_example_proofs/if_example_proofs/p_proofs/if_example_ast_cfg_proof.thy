...
proof -
  have "ast_cfg_rel None [] bigblock0 p_before_cfg_to_dag_prog.block_0" 
...
proof -
  have syntactic_rel: "ast_cfg_rel None [] bigblock_then [(Assign 0 (Lit (LInt 10)))]" 
    by (rule Rel_Main_test)
  show ?thesis
    unfolding p_before_cfg_to_dag_prog.block_3_def 
...
proof -
  have syntactic_rel: "ast_cfg_rel None [] bigblock_else [(Assign 0 (Lit (LInt 1)))]"
    by (rule Rel_Main_test)
  have guard_equiv: "UnOp Not (BinOp (Var 0) Gt (Lit (LInt 5))) \<sim> (Lit (LInt 5) \<guillemotleft>Ge\<guillemotright> Var 0)" 
    by (rule neg_gt2)
  show ?thesis
    unfolding p_before_cfg_to_dag_prog.block_1_def 
    apply (rule block_local_rel_guard_false[OF syntactic_rel _ _ guard_equiv _ trace_is_possible Red_bb0_to Red0_impl])
    unfolding p_before_cfg_to_dag_prog.block_1_def
    by simp_all
qed
...
lemma block2_global_rel:
  ...
  using assms
proof -
  have syn_rel: "ast_cfg_rel None [] bigblock1 [(Assert (BinOp (Var 0) Gt (Lit (LInt 0))))]" using Rel_Main_test by blast 
  have cmds: "node_to_block p_before_cfg_to_dag_prog.proc_body ! 2 = [(Assert (BinOp (Var 0) Gt (Lit (LInt 0))))]" 
...
lemma block_then_global_rel:
  ...
  shows "(Ast.valid_configuration A \<Lambda> \<Gamma> \<Omega> [] reached_bb reached_cont reached_state)" 
proof -
  have syntactic_rel: "ast_cfg_rel None [] (BigBlock None [(Assign 0 (Lit (LInt 10)))] None None) [(Assign 0 (Lit (LInt 10)))]"
    by (simp add: Rel_Main_test)
  have succ: "(out_edges(p_before_cfg_to_dag_prog.proc_body) ! 3) = [2]" 
    by (simp add: p_before_cfg_to_dag_prog.outEdges_3) 
...
proof -
  have syntactic_rel: "ast_cfg_rel None [] bigblock_else [(Assign 0 (Lit (LInt 1)))]"
    by (simp add: Rel_Main_test)
  have succ: "(out_edges (p_before_cfg_to_dag_prog.proc_body) ! Suc 0) = [2]" 
    using p_before_cfg_to_dag_prog.outEdges_1 by auto 
  have guard_equiv: "UnOp Not (BinOp (Var 0) Gt (Lit (LInt 5))) \<sim> (Lit (LInt 5) \<guillemotleft>Ge\<guillemotright> Var 0)" 
    by (rule neg_gt2)
...
lemma block0_global_rel:
  ...
  shows "(valid_configuration A \<Lambda> \<Gamma> \<Omega> p_before_cfg_to_dag_prog.post reached_bb reached_cont reached_state)" 
proof -
  have cmds: "node_to_block(p_before_cfg_to_dag_prog.proc_body) ! 0 = [(Havoc 0)]" 
    using p_before_cfg_to_dag_prog.block_0_def p_before_cfg_to_dag_prog.node_0 by auto
  have syntactic_rel: "ast_cfg_rel None [] bigblock0 [Havoc 0]"  
    by (simp add: Rel_Main_test)
  have succ: "(out_edges(p_before_cfg_to_dag_prog.proc_body) ! 0) = [3, 1]" 
    using p_before_cfg_to_dag_prog.outEdges_0 by auto 
  have "Suc 0 = 1" by simp
  show ?thesis 
...
assumes 
Red: "rtranclp (red_bigblock A M ((append global_data.constants_vdecls global_data.globals_vdecls),(append p_before_cfg_to_dag_prog.params_vdecls p_before_cfg_to_dag_prog.locals_vdecls)) \<Gamma> [] if_example_before_ast_cfg.proc_body) (bigblock0, (KSeq bigblock1 KStop), Normal ns) (end_bb, end_cont, end_state)" and 
...
shows "(valid_configuration A \<Lambda>0 \<Gamma> [] p_before_cfg_to_dag_prog.post end_bb end_cont end_state)"
proof -
from Red obtain j where Aux:"(A,M,((append global_data.constants_vdecls global_data.globals_vdecls),(append p_before_cfg_to_dag_prog.params_vdecls p_before_cfg_to_dag_prog.locals_vdecls)),\<Gamma>,[],if_example_before_ast_cfg.proc_body \<turnstile> (bigblock0, (KSeq bigblock1 KStop), Normal ns) -n\<longrightarrow>^j (end_bb, end_cont, end_state))"
...
proof -
  have "rtranclp (red_bigblock A M \<Lambda> \<Gamma> \<Omega> T) (bigblock0, convert_list_to_cont (rev [bigblock1]) KStop, Normal ns1) (reached_bb, reached_cont, reached_state)"
...
apply assumption using VC apply simp  apply assumption+
       apply (simp_all add: 
              ...
              if_example_before_ast_cfg.pres_def if_example_before_ast_cfg.post_def
              p_before_cfg_to_dag_prog.params_vdecls_def p_before_cfg_to_dag_prog.locals_vdecls_def
              if_example_before_ast_cfg.params_vdecls_def if_example_before_ast_cfg.locals_vdecls_def
              p_before_cfg_to_dag_prog.pres_def p_before_cfg_to_dag_prog.post_def)
       done
