...
definition block_1
  where
    "block_1  = [(Assume (BinOp (Var 1) Gt (Lit (LInt 0)))),(Assume (BinOp (Var 2) Eq (BinOp (Var 1) Sub (Lit (LInt 1)))))]"
definition block_2
  where
    "block_2  = [(Assume (BinOp (Var 1) Lt (Lit (LInt 0)))),(Assume (BinOp (Var 3) Eq (BinOp (Var 1) Add (Lit (LInt 1)))))]"
definition block_3
  where
    "block_3  = [(Assert (BinOp (Lit (LInt 0)) Eq (Lit (LInt 0))))]"
definition block_4
  where
    "block_4  = [(Assume (BinOp (Lit (LInt 0)) Le (Var 1)))]"
definition block_5
  where
    "block_5  = [(Assert (BinOp (Var 1) Le (Lit (LInt 0))))]"
definition block_6
  where
    "block_6  = []"
definition block_7
...
lemma m_x_2:
shows "((map_of (append return_in_loop_passive_prog.params_vdecls return_in_loop_passive_prog.locals_vdecls) 3) = (Some ((TPrim TInt),(None ))))"
by (simp add:params_vdecls_def locals_vdecls_def)
