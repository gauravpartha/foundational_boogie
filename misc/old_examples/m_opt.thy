...
lemma block1_correct:
  assumes 
   A1:"\<Gamma> \<turnstile> \<langle>[                  
                 (Assert (BinOp (FunExp ''f'' [(Var ''x'')]) Le (Var ''y'')))
                ], Normal n_s\<rangle> [\<rightarrow>] s'" and
 ...
 using assms
proof (cases) 
  case (RedListCons n_s') 
  have "\<Gamma> \<turnstile> \<langle>(BinOp (FunExp ''f'' [(Var ''x'')]) Le (Var ''y'')), n_s\<rangle> \<Down> (BoolV True)"
    by ( (simp add: assms | (rule+)), (simp add: assms)?)+
  from A1 show ?thesis
  proof cases
    case RedListNil
      thus ?thesis by simp
    case (RedListCons c s'' cs)
    hence Asame:"s'' = s'" by (simp add: step_nil_same) (* since cs = [] *)
    (*hence A7: "c =  (Assert (BinOp (FunExp ''f'' [(Var ''x'')]) Le (Var ''y'')))" by simp*)
    assume "\<Gamma> \<turnstile> \<langle>c, Normal n_s\<rangle> \<rightarrow> s''"
  ...
  next
    ...
    proof cases
      ...
      have "s'' \<noteq> Failure \<and> (\<forall> n_s'. s'' = Normal n_s' \<longrightarrow> ( (f' ix \<le> 5 \<and> 6 \<le> iy) \<and> n_s' = n_s))"       
        thm block0_correct
      (* maybe get rid of separate lemma here *)
