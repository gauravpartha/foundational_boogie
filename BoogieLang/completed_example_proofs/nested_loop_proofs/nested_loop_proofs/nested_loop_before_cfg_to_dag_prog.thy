...
definition block_5
  where
    "block_5  = [(Assume (BinOp (Lit (LInt 0)) Ge (Var 1))),(Assign 0 (BinOp (Var 0) Sub (Lit (LInt 1))))]"
definition block_6
  where
    "block_6  = [(Assume (BinOp (Lit (LInt 0)) Ge (Var 0)))]"
definition outEdges
  where
    "outEdges  = [[1],[6,2],[3],[5,4],[3],[1],[]]"
...
shows "((Set.inter (set (map fst (append global_data.constants_vdecls global_data.globals_vdecls))) (set (map fst (append nested_loop_before_cfg_to_dag_prog.params_vdecls nested_loop_before_cfg_to_dag_prog.locals_vdecls)))) = {})"
...
shows "(\<forall> x \<tau>. (((lookup_var_ty ((append global_data.constants_vdecls global_data.globals_vdecls),(append nested_loop_before_cfg_to_dag_prog.params_vdecls nested_loop_before_cfg_to_dag_prog.locals_vdecls)) x) = (Some \<tau>)) \<longrightarrow> ((wf_ty 0) \<tau>)))"
