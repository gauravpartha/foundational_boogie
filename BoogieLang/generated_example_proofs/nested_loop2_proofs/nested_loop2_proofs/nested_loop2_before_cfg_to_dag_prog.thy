...
definition block_7
  where
    "block_7  = [(Assume (BinOp (Lit (LInt 0)) Ge (Var 1))),(Assign 0 (BinOp (Var 0) Sub (Lit (LInt 1))))]"
definition block_8
  where
    "block_8  = [(Assume (BinOp (Lit (LInt 0)) Ge (Var 0)))]"
definition block_9
  where
    "block_9  = [(Assume (UnOp Not (Lit (LBool True))))]"
definition outEdges
  where
    "outEdges  = [[1],[9,2],[3],[8,4],[5],[7,6],[5],[3],[1],[]]"
...
lemma outEdges_3:
shows "((nth (out_edges proc_body) 3) = [8,4])"
by (simp add:proc_body_def outEdges_def)
...
lemma outEdges_7:
shows "((nth (out_edges proc_body) 7) = [3])"
by (simp add:proc_body_def outEdges_def)
...
shows "((Set.inter (set (map fst (append global_data.constants_vdecls global_data.globals_vdecls))) (set (map fst (append nested_loop2_before_cfg_to_dag_prog.params_vdecls nested_loop2_before_cfg_to_dag_prog.locals_vdecls)))) = {})"
...
shows "(\<forall> x \<tau>. (((lookup_var_ty ((append global_data.constants_vdecls global_data.globals_vdecls),(append nested_loop2_before_cfg_to_dag_prog.params_vdecls nested_loop2_before_cfg_to_dag_prog.locals_vdecls)) x) = (Some \<tau>)) \<longrightarrow> ((wf_ty 0) \<tau>)))"
