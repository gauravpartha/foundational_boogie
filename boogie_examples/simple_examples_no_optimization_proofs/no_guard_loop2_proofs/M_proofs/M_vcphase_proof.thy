...
fixes A :: "(('a)absval_ty_fun)" and \<Lambda> :: "(var_context)" and \<Gamma> :: "(('a)fun_interp)" and n_s :: "(('a)nstate)" and vc_x :: "int" and vc_y :: "int" and vc_z :: "int" and vc_x_0 :: "int" and vc_y_0 :: "int" and vc_z_0 :: "int" and vc_x_1 :: "int" and vc_y_1 :: "int" and vc_x_2 :: "int" and vc_x_3 :: "int" and vc_y_2 :: "int" and vc_x_4 :: "int"
assumes 
...
ML\<open>
val block_anon9_Then_hints = [
(AssumeTrue,NONE), 
(AssumeTrue,NONE)]
\<close>
lemma block_anon9_ThenAA0:
...
apply (tactic \<open> boogie_vc_tac @{context} @{thms global_assms} (@{thm forall_poly_thm}, @{thm exists_poly_thm}) block_anon9_Then_hints \<close>)
by (auto?)

ML\<open>
val block_anon9_Else_hints = [
(AssumeTrue,NONE)]
\<close>
lemma block_anon9_ElseAA0:
shows "((red_cmd_list A M \<Lambda> \<Gamma> \<Omega> M_passive_prog.block_5 (Normal n_s) s') \<Longrightarrow> ((vc.vc_anon0 ) \<Longrightarrow> ((s' \<noteq> Failure) \<and> (\<forall> n_s'. ((s' = (Normal n_s')) \<longrightarrow> ((n_s' = n_s) \<and> (vc.vc_anon0 )))))))"
unfolding M_passive_prog.block_5_def vc.vc_anon0_def
apply (tactic \<open> boogie_vc_tac @{context} @{thms global_assms} (@{thm forall_poly_thm}, @{thm exists_poly_thm}) block_anon9_Else_hints \<close>)
by (auto?)
...
shows "((red_cmd_list A M \<Lambda> \<Gamma> \<Omega> M_passive_prog.block_7 (Normal n_s) s') \<Longrightarrow> ((vc.vc_anon0 ) \<Longrightarrow> ((s' \<noteq> Failure) \<and> (\<forall> n_s'. ((s' = (Normal n_s')) \<longrightarrow> ((n_s' = n_s) \<and> ((vc.vc_anon0 ) \<and> (vc.vc_anon0 ))))))))"
unfolding M_passive_prog.block_7_def vc.vc_anon0_def
...
shows "((red_cmd_list A M \<Lambda> \<Gamma> \<Omega> M_passive_prog.block_13 (Normal n_s) s') \<Longrightarrow> ((vc.vc_anon0 ) \<Longrightarrow> ((s' \<noteq> Failure) \<and> (\<forall> n_s'. ((s' = (Normal n_s')) \<longrightarrow> ((n_s' = n_s) \<and> ((vc.vc_anon0 ) \<and> (vc.vc_anon0 ))))))))"
apply (erule red_cmd_list.cases)
using global_assms
unfolding M_passive_prog.block_13_def vc.vc_anon0_def
...
apply (erule block_anon9_ThenAA0[OF _ assms(2)])
...
apply (erule block_anon9_ElseAA0[OF _ assms(2)])
...
apply (erule member_elim, simp)
apply (erule cfg_block_anon9_Then, simp?)
apply (erule member_elim, simp)
apply (erule cfg_block_anon9_Else, simp?)
by (simp add: member_rec(2))
...
apply (erule block_anon7_LoopBody[OF _ assms(2)])
...
lemma end_to_end:
assumes 
Red: "(red_cfg_multi A M ((append M_before_ast_to_cfg_prog.constants_vdecls M_before_ast_to_cfg_prog.globals_vdecls),(append M_passive_prog.params_vdecls M_passive_prog.locals_vdecls)) \<Gamma> [] M_passive_prog.proc_body ((Inl 13),(Normal n_s)) (m',s'))" and 
