theory nested_loop_before_ast_cfg
  imports
    Boogie_Lang.Ast
...
shows "(((map fst (append nested_loop_before_ast_cfg.params_vdecls nested_loop_before_ast_cfg.locals_vdecls)) \<noteq> []) \<longrightarrow> ((Min (set (map fst (append nested_loop_before_ast_cfg.params_vdecls nested_loop_before_ast_cfg.locals_vdecls)))) \<ge> 0))"
unfolding nested_loop_before_ast_cfg.params_vdecls_def nested_loop_before_ast_cfg.locals_vdecls_def
by simp

lemma locals_min:
shows "(\<forall> x. ((Set.member x (set (map fst (append nested_loop_before_ast_cfg.params_vdecls nested_loop_before_ast_cfg.locals_vdecls)))) \<longrightarrow> (x \<ge> 0)))"
using locals_min_aux helper_min
by blast

lemma globals_locals_disj:
shows "((Set.inter (set (map fst (append global_data.constants_vdecls global_data.globals_vdecls))) (set (map fst (append nested_loop_before_ast_cfg.params_vdecls nested_loop_before_ast_cfg.locals_vdecls)))) = {})"
...
lemma params_wf:
shows "((list_all (comp (wf_ty 0) (comp fst snd)) nested_loop_before_ast_cfg.params_vdecls) )"
unfolding nested_loop_before_ast_cfg.params_vdecls_def
by simp

lemma locals_wf:
shows "((list_all (comp (wf_ty 0) (comp fst snd)) nested_loop_before_ast_cfg.locals_vdecls) )"
unfolding nested_loop_before_ast_cfg.locals_vdecls_def
by simp

lemma var_context_wf:
shows "(\<forall> x \<tau>. (((lookup_var_ty ((append global_data.constants_vdecls global_data.globals_vdecls),(append nested_loop_before_ast_cfg.params_vdecls nested_loop_before_ast_cfg.locals_vdecls)) x) = (Some \<tau>)) \<longrightarrow> ((wf_ty 0) \<tau>)))"
apply (rule lookup_ty_pred_2)
by ((simp_all add:consts_wf globals_wf params_wf locals_wf))

lemma m_x:
shows "((map_of (append nested_loop_before_ast_cfg.params_vdecls nested_loop_before_ast_cfg.locals_vdecls) 0) = (Some ((TPrim TInt),(None ))))"
by (simp add:params_vdecls_def locals_vdecls_def)

lemma m_y:
shows "((map_of (append nested_loop_before_ast_cfg.params_vdecls nested_loop_before_ast_cfg.locals_vdecls) 1) = (Some ((TPrim TInt),(None ))))"
by (simp add:params_vdecls_def locals_vdecls_def)
