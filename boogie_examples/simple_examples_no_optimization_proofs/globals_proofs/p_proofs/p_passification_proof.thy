...
by ((tactic \<open> cfg_lemma_tac @{context} @{thm assms(1)} @{thm assms(2)} @{thms assms(3-)} (@{thm p_before_passive_prog.node_0},@{thm p_before_passive_prog.outEdges_0}) (@{thm p_passive_prog.node_0},@{thm p_passive_prog.outEdges_0}) @{thm block_anon0} [] 1\<close>))
...
by ((tactic \<open> cfg_lemma_tac @{context} @{thm assms(1)} @{thm assms(2)} @{thms assms(3-)} (@{thm p_before_passive_prog.node_1},@{thm p_before_passive_prog.outEdges_1}) (@{thm p_passive_prog.node_1},@{thm p_passive_prog.outEdges_1}) @{thm block_0} [
...
by ((tactic \<open> cfg_lemma_tac @{context} @{thm assms(1)} @{thm assms(2)} @{thms assms(3-)} (@{thm p_before_passive_prog.node_2},@{thm p_before_passive_prog.outEdges_2}) (@{thm p_passive_prog.node_2},@{thm p_passive_prog.outEdges_2}) @{thm block_PreconditionGeneratedEntry} [
...
assumes 
Red: "(red_cfg_multi A M ((append p_before_ast_to_cfg_prog.constants_vdecls p_before_ast_to_cfg_prog.globals_vdecls),(append p_before_ast_to_cfg_prog.params_vdecls p_before_ast_to_cfg_prog.locals_vdecls)) \<Gamma> [] p_before_passive_prog.proc_body ((Inl 2),(Normal ns)) (m',s'))" and 
...
apply ((intro conjI)?)
apply ((simp add:p_before_ast_to_cfg_prog.l_a(2) p_passive_prog.l_a(2)))
apply ((simp add:p_before_ast_to_cfg_prog.l_b(2) p_passive_prog.l_b(2)))
done
...
proof
assume A1: "(s' = Failure)"
have "((s' = Failure) \<longrightarrow> (\<exists> u. ((Set.member u U0) \<and> (passive_sim_cfg_fail A M \<Lambda>2 \<Gamma> [] p_passive_prog.proc_body u (Inl 2)))))"
...
with A1 obtain u mp' where uElem: "(Set.member u U0)" and AredPassive:"(red_cfg_multi A M \<Lambda>2 \<Gamma> [] p_passive_prog.proc_body ((Inl 2),(Normal u)) (mp',Failure))"
