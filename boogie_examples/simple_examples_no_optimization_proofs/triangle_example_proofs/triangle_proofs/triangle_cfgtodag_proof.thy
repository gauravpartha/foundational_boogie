...
imports Boogie_Lang.Semantics Boogie_Lang.Util Boogie_Lang.BackedgeElim Boogie_Lang.TypingML triangle_before_ast_to_cfg_prog triangle_before_cfg_to_dag_prog triangle_before_passive_prog triangle_passification_proof triangle_vcphase_proof
begin
...
shows "(expr_all_sat A \<Lambda>1 \<Gamma> [] ns2 triangle_before_ast_to_cfg_prog.post)"
unfolding expr_all_sat_def triangle_before_ast_to_cfg_prog.post_def 
apply (rule cfg_dag_rel_post_invs_3)
apply (erule assms(1))
apply (rule triangle_before_passive_prog.node_0)
apply simp
apply (rule type_safety_top_level_inv[OF Wf_Fun triangle_before_ast_to_cfg_prog.funcs_wf triangle_before_ast_to_cfg_prog.var_context_wf])
apply (rule assms(2))
apply (simp)
apply ((tactic \<open> typing_tac @{context} @{thms ty_hint_1} @{thms triangle_before_ast_to_cfg_prog.l_t(2) triangle_before_ast_to_cfg_prog.l_n(2)} [] 1\<close>))
...
shows "(Semantics.valid_configuration A \<Lambda>1 \<Gamma> [] triangle_before_ast_to_cfg_prog.post m' s')"
...
apply simp
apply (intro conjI)
apply (erule type_safety_top_level_inv[OF Wf_Fun triangle_before_ast_to_cfg_prog.funcs_wf triangle_before_ast_to_cfg_prog.var_context_wf])
apply (simp)
apply ((tactic \<open> typing_tac @{context} @{thms ty_hint_2} @{thms triangle_before_ast_to_cfg_prog.l_t(2) triangle_before_ast_to_cfg_prog.l_m(2)} [] 1\<close>))
apply (erule type_safety_top_level_inv[OF Wf_Fun triangle_before_ast_to_cfg_prog.funcs_wf triangle_before_ast_to_cfg_prog.var_context_wf])
apply (simp)
apply ((tactic \<open> typing_tac @{context} [] @{thms triangle_before_ast_to_cfg_prog.l_m(2) triangle_before_ast_to_cfg_prog.l_n(2)} [] 1\<close>))
...
IH_anon2_LoopHead: "(loop_ih A M \<Lambda>1 \<Gamma> [] triangle_before_cfg_to_dag_prog.proc_body [2,1] [(BinOp (Var 2) Eq (BinOp (BinOp (Var 1) Mul (BinOp (Var 1) Sub (Lit (LInt 1)))) Div (Lit (LInt 2)))),(BinOp (Var 1) Le (Var 0))] triangle_before_ast_to_cfg_prog.post ns1 s' 1 m' j)"
shows "(Semantics.valid_configuration A \<Lambda>1 \<Gamma> [] triangle_before_ast_to_cfg_prog.post m' s')"
...
apply cfg_dag_rel_tac_single+
apply simp
apply ((simp add:triangle_before_ast_to_cfg_prog.l_t(1)))
apply ((simp add:triangle_before_ast_to_cfg_prog.l_m(1)))
apply simp
done
...
apply (erule type_safety_top_level_inv[OF Wf_Fun triangle_before_ast_to_cfg_prog.funcs_wf triangle_before_ast_to_cfg_prog.var_context_wf])
apply (simp)
apply ((tactic \<open> typing_tac @{context} @{thms ty_hint_3} @{thms triangle_before_ast_to_cfg_prog.l_t(2) triangle_before_ast_to_cfg_prog.l_m(2)} [] 1\<close>))
apply (erule type_safety_top_level_inv[OF Wf_Fun triangle_before_ast_to_cfg_prog.funcs_wf triangle_before_ast_to_cfg_prog.var_context_wf])
apply (simp)
apply ((tactic \<open> typing_tac @{context} [] @{thms triangle_before_ast_to_cfg_prog.l_m(2) triangle_before_ast_to_cfg_prog.l_n(2)} [] 1\<close>))
...
"(expr_all_sat A \<Lambda>1 \<Gamma> [] ns2 triangle_before_ast_to_cfg_prog.pres)"
shows "(Semantics.valid_configuration A \<Lambda>1 \<Gamma> [] triangle_before_ast_to_cfg_prog.post m' s')"
...
apply (rule assume_pres_normal[where ?es=triangle_before_ast_to_cfg_prog.pres])
apply (rule assms(4))
unfolding triangle_before_ast_to_cfg_prog.pres_def
apply simp
...
assumes 
Red: "(red_cfg_multi A M ((append triangle_before_ast_to_cfg_prog.constants_vdecls triangle_before_ast_to_cfg_prog.globals_vdecls),(append triangle_before_ast_to_cfg_prog.params_vdecls triangle_before_ast_to_cfg_prog.locals_vdecls)) \<Gamma> [] triangle_before_cfg_to_dag_prog.proc_body ((Inl 0),(Normal ns)) (m',s'))" and 
...
shows "(Semantics.valid_configuration A \<Lambda>0 \<Gamma> [] triangle_before_ast_to_cfg_prog.post m' s')"
proof -
from Red obtain j where Aux:"(red_cfg_k_step A M ((append triangle_before_ast_to_cfg_prog.constants_vdecls triangle_before_ast_to_cfg_prog.globals_vdecls),(append triangle_before_ast_to_cfg_prog.params_vdecls triangle_before_ast_to_cfg_prog.locals_vdecls)) \<Gamma> [] triangle_before_cfg_to_dag_prog.proc_body ((Inl 0),(Normal ns)) j (m',s'))"
