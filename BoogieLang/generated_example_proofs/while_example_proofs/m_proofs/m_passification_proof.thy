...
imports Boogie_Lang.Semantics Boogie_Lang.Util m_before_passive_prog m_passive_prog Boogie_Lang.PassificationML m_vcphase_proof Boogie_Lang.PassificationEndToEnd
begin
...
abbreviation \<Lambda>2
  where
    "\<Lambda>2  \<equiv> ((append global_data.constants_vdecls global_data.globals_vdecls),(append m_passive_prog.params_vdecls m_passive_prog.locals_vdecls))"
...
"((R 0) = (Some (Inl 2)))" and 
"((R 1) = (Some (Inl 1)))"
shows "(passive_block_conclusion A M \<Lambda>1 \<Lambda>2 \<Gamma> \<Omega> U0 (Set.union D0 (set [3])) (update_nstate_rel R [(0,(Inl 3))]) R_old m_passive_prog.block_2 s')"
apply (rule passification_block_lemma_compact[OF assms(1-2)])
unfolding m_before_passive_prog.block_2_def m_passive_prog.block_2_def
apply (passive_rel_tac R_def: assms(3-))
apply (unfold type_rel_def, simp, (intro conjI)?)
apply ((simp add:m_before_cfg_to_dag_prog.l_i(2) m_passive_prog.l_i_1(2)))
by simp
...
"((R 1) = (Some (Inl 1)))"
shows "(passive_block_conclusion A M \<Lambda>1 \<Lambda>2 \<Gamma> \<Omega> U0 (Set.union D0 (set [2])) (update_nstate_rel R [(0,(Inl 2))]) R_old m_passive_prog.block_3 s')"
apply (rule passification_block_lemma_compact[OF assms(1-2)])
unfolding m_before_passive_prog.block_3_def m_passive_prog.block_3_def
apply (passive_rel_tac R_def: assms(3-))
apply (unfold type_rel_def, simp, (intro conjI)?)
apply ((simp add:m_before_cfg_to_dag_prog.l_i(2) m_passive_prog.l_i_0(2)))
by simp
...
"(passive_lemma_assms A M \<Lambda>1 \<Lambda>2 \<Gamma> \<Omega> [] R R_old U0 D0 n_s)" and 
"((R 1) = (Some (Inl 1)))"
shows "(passive_block_conclusion A M \<Lambda>1 \<Lambda>2 \<Gamma> \<Omega> U0 (Set.union D0 (set [])) (update_nstate_rel R [(0,(Inr (LInt 0)))]) R_old m_passive_prog.block_4 s')"
apply (rule passification_block_lemma_compact[OF assms(1-2)])
...
by ((tactic \<open> cfg_lemma_tac @{context} @{thm assms(1)} @{thm assms(2)} @{thms assms(3-)} (@{thm m_before_passive_prog.node_1},@{thm m_before_passive_prog.outEdges_1}) (@{thm m_passive_prog.node_1},@{thm m_passive_prog.outEdges_1}) @{thm block_anon3_LoopDone} [
...
by ((tactic \<open> cfg_lemma_tac @{context} @{thm assms(1)} @{thm assms(2)} @{thms assms(3-)} (@{thm m_before_passive_prog.node_2},@{thm m_before_passive_prog.outEdges_2}) (@{thm m_passive_prog.node_2},@{thm m_passive_prog.outEdges_2}) @{thm block_anon3_LoopBody} [
@{thm cfg_block_GeneratedUnifiedExit}] 1\<close>))
...
by ((tactic \<open> cfg_lemma_tac @{context} @{thm assms(1)} @{thm assms(2)} @{thms assms(3-)} (@{thm m_before_passive_prog.node_3},@{thm m_before_passive_prog.outEdges_3}) (@{thm m_passive_prog.node_3},@{thm m_passive_prog.outEdges_3}) @{thm block_anon3_LoopHead} [
...
by ((tactic \<open> cfg_lemma_tac @{context} @{thm assms(1)} @{thm assms(2)} @{thms assms(3-)} (@{thm m_before_passive_prog.node_4},@{thm m_before_passive_prog.outEdges_4}) (@{thm m_passive_prog.node_4},@{thm m_passive_prog.outEdges_4}) @{thm block_anon0} [
...
by ((tactic \<open> cfg_lemma_tac @{context} @{thm assms(1)} @{thm assms(2)} @{thms assms(3-)} (@{thm m_before_passive_prog.node_5},@{thm m_before_passive_prog.outEdges_5}) (@{thm m_passive_prog.node_5},@{thm m_passive_prog.outEdges_5}) @{thm block_0} [
...
by ((tactic \<open> cfg_lemma_tac @{context} @{thm assms(1)} @{thm assms(2)} @{thms assms(3-)} (@{thm m_before_passive_prog.node_6},@{thm m_before_passive_prog.outEdges_6}) (@{thm m_passive_prog.node_6},@{thm m_passive_prog.outEdges_6}) @{thm block_PreconditionGeneratedEntry} [
...
assumes 
Red: "(red_cfg_multi A M ((append global_data.constants_vdecls global_data.globals_vdecls),(append m_before_cfg_to_dag_prog.params_vdecls m_before_cfg_to_dag_prog.locals_vdecls)) \<Gamma> [] m_before_passive_prog.proc_body ((Inl 6),(Normal ns)) (m',s'))" and 
...
apply ((intro conjI)?)
apply ((simp add:m_before_cfg_to_dag_prog.l_n(2) m_passive_prog.l_n(2)))
done
...
proof
assume A1: "(s' = Failure)"
have "((s' = Failure) \<longrightarrow> (\<exists> u. ((Set.member u U0) \<and> (passive_sim_cfg_fail A M \<Lambda>2 \<Gamma> [] m_passive_prog.proc_body u (Inl 6)))))"
...
with A1 obtain u mp' where uElem: "(Set.member u U0)" and AredPassive:"(red_cfg_multi A M \<Lambda>2 \<Gamma> [] m_passive_prog.proc_body ((Inl 6),(Normal u)) (mp',Failure))"
