theory nested_loop2_before_passive_prog
imports Boogie_Lang.Semantics Boogie_Lang.Util nested_loop2_before_cfg_to_dag_prog
begin
definition block_0
  where
    "block_0  = []"
...
definition block_2
  where
    "block_2  = [(Assume (BinOp (Lit (LInt 0)) Ge (Var 0))),(Assume (Lit (LBool False)))]"
definition block_3
  where
    "block_3  = [(Assume (BinOp (Lit (LInt 0)) Ge (Var 1))),(Assign 0 (BinOp (Var 0) Sub (Lit (LInt 1)))),(Assert (BinOp (Var 0) Ge (Lit (LInt 0)))),(Assume (Lit (LBool False)))]"
definition block_4
  where
    "block_4  = [(Assume (BinOp (Var 1) Gt (Lit (LInt 0)))),(Assign 1 (BinOp (Var 1) Sub (Lit (LInt 1)))),(Assert (BinOp (Var 1) Ge (Lit (LInt 0)))),(Assume (Lit (LBool False)))]"
definition block_5
  where
    "block_5  = [(Havoc 1),(Assume (BinOp (Var 1) Ge (Lit (LInt 0))))]"
definition block_6
  where
    "block_6  = [(Assume (BinOp (Var 0) Gt (Lit (LInt 0)))),(Assert (BinOp (Var 1) Ge (Lit (LInt 0))))]"
definition block_7
  where
    "block_7  = [(Havoc 0),(Havoc 1),(Assume (BinOp (Var 0) Ge (Lit (LInt 0))))]"
definition block_8
  where
    "block_8  = [(Assume (Lit (LBool True))),(Assert (BinOp (Var 0) Ge (Lit (LInt 0))))]"
definition block_9
  where
    "block_9  = [(Havoc 0),(Havoc 1)]"
definition block_10
  where
    "block_10  = [(Assign 0 (Lit (LInt 10))),(Assign 1 (Lit (LInt 10)))]"
definition block_11
  where
    "block_11  = []"
definition block_12
